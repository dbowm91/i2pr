...
| `src/service_product.rs` | Plan 209 single production composition helper wiring SSU2 + tunnels + LeaseSet2 + backend | `ServiceProduct::start`, `ServiceProduct::poll_inbound`, `ServiceProduct::remote_counters` |
| `src/service_tunnels.rs` | M10 `ServiceTunnelManager` with per-service destination runtime and listener lifecycle | `ServiceTunnelManager`, `register_service_tunnel_manager` |
| `src/service_tunnels_http.rs` | HTTP proxy executor (one loopback listener per `http-client` spec) | HTTP executor types |
| `src/service_tunnels_socks5.rs` | SOCKS5 executor (one loopback listener per `socks5-client` spec) | SOCKS5 executor types |
| `src/service_tunnels_irc_client.rs` | IRC client executor (one loopback listener per `irc-client` spec) | IRC client executor types |
