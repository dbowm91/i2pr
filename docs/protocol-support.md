...
| Reseed and RouterInfo publication | Plans 103–107 implemented locally on this host: RouterInfo validation, bounded local NetDB, persistent cache, SU3 reseed, transport-neutral query state machines, daemon bootstrap integration, and the Milestone 5 exploratory tunnel substrate. Plan 108 landed the ECIES-X25519 short tunnel-build architecture but its wire/cryptographic algorithm was not protocol-conformant against the current official I2P Tunnel Creation Specification (see `plans/108-conformance-amendment.md`). Plan 109 corrected the wire format, Noise-N transcript, layer-encryption type, request/reply key derivation, response codes, and 218-byte envelope layout but missed four defects that Plan 111 reopens. Plan 110 added randomized slot allocation, fake records, raw ChaCha20 preprocessing/postprocessing, and the one-byte-count STBM/OTBRM payload framing but inherited the Plan 109 defects. Plan 111 closed as `passed-final-local-short-build-conformance` and corrected the remaining defects (Noise null prologue, single-HKDF `es` split, slot byte at offset 4, 8-byte OBEP garlic tag, explicit per-hop tunnel IDs, role-aware hop processor, frozen independent fixed vectors); inbound creator-ephemeral layout remains `blocked-inbound-layout-ambiguity` until a current reference-router source pins the placement. Plan 112 lands the outbound pre-delivery closure: CSPRNG-filled post-Mapping padding, direction/role topology validation, the typed `InboundBuildPendingReconciliation` fail-closed gate that holds inbound production construction, and the Rust-only reference provenance test under `crates/i2pr-tunnel/tests/plan111_reference_vectors.rs` | 4/5 | `specs/protocols/04-reseed-netdb.md`, `specs/protocols/05-tunnels.md`, `plans/103-routerinfo-validation-and-local-netdb-foundation.md`, `plans/104-persistent-netdb-cache-and-su3-reseed-trust-path.md`, `plans/105-transport-neutral-netdb-query-and-publication-state-machines.md`, `plans/106-daemon-netdb-bootstrap-integration-and-milestone-5-handoff.md`, `plans/107-milestone-5-exploratory-tunnel-substrate.md`, `plans/108-live-ecies-x25519-short-tunnel-build-construction.md`, `plans/108-conformance-amendment.md`, `plans/109-short-build-record-and-noise-conformance-correction.md`, `plans/110-short-build-multirecord-preprocessing-and-conformance-closure.md`, `plans/111-short-build-final-local-conformance-correction.md`, `plans/112-113-post-plan111-pre-delivery-corrective-roadmap.md`, `plans/112-outbound-short-build-pre-delivery-closure.md`, `plans/103-status.md`, `plans/104-status.md`, `plans/105-status.md`, `plans/106-status.md`, `plans/107-status.md`, `plans/108-status.md`, `plans/109-status.md`, `plans/110-status.md`, `plans/111-status.md`, `plans/112-status.md` | Local signed-region verification, fresh/future/malformed/stale rejection, store replacement/conflict/quota/prune, SU3 trust + bounded ZIP ingestion, offline reseed through the daemon bootstrap pipeline, Plan 105 transport-neutral lookup state machines, Plan 106 runtime-facing NetDB seam, Plan 107 exploratory tunnel pool + reply-path provider + build-record layout + build-cryptography seam, Plan 108 ECIES-X25519 short request/reply records + per-hop crypto + runtime-neutral build state machine + success-only `ShortBuildRegistrar`, Plan 109 corrected wire/crypto layout, Plan 110 added randomized slot allocation + fake records + raw ChaCha20 preprocessing/postprocessing + one-byte-count STBM/OTBRM payload framing, Plan 111 final local short-build conformance correction (Noise null prologue, single-HKDF `es`, slot byte at offset 4, 8-byte OBEP garlic tag, explicit per-hop tunnel IDs, role-aware hop processor, frozen independent fixed vectors; inbound creator-ephemeral layout remains blocked), Plan 112 outbound pre-delivery closure (CSPRNG-filled post-Mapping padding, direction/role topology validation, typed `InboundBuildPendingReconciliation` fail-closed gate, Rust-only reference provenance test, explicit `validate_count_prefixed_short_payload`/`encode_count_prefixed_short_payload` STBM/OTBRM contract helpers); the live `RouterInfo` lookup over exploratory tunnels still depends on the qualified transport and the external mixed-router validation lane | None |
| Network tunnels and transit participation | Experimental local subset; outbound short tunnel-build locally conformant against fixed vectors and gated against the Plan 112 direction/role topology validator, inbound construction blocked on layout ambiguity and gated by the typed `InboundBuildPendingReconciliation` fail-closed gate, exploratory substrate implemented, live mixed-router build pending | 5 | `specs/protocols/05-tunnels.md`, `plans/107-milestone-5-exploratory-tunnel-substrate.md`, `plans/108-live-ecies-x25519-short-tunnel-build-construction.md`, `plans/108-conformance-amendment.md`, `plans/109-short-build-record-and-noise-conformance-correction.md`, `plans/110-short-build-multirecord-preprocessing-and-conformance-closure.md`, `plans/111-short-build-final-local-conformance-correction.md`, `plans/112-113-post-plan111-pre-delivery-corrective-roadmap.md`, `plans/112-outbound-short-build-pre-delivery-closure.md`, `plans/112-status.md` | Bounded `TunnelId`/`TunnelRole`/`TunnelLifetime`/`TunnelState`/`TunnelPeer` types, bounded `ExploratoryPoolConfig` with documented hard ceilings, deterministic `ExploratoryPool` with replacement/expiry/failure accounting, `BuildRecordLayout` (Short/Variable) over `DeferredBuildRecords`, typed `BuildCryptography` seam with `NoBuildCryptography` default, `EciesX25519BuildCryptography` Plan 111 corrected implementation that seals a 154-byte plaintext into a 218-byte record and authenticates/decrypts the 202-byte reply (Plan 111 protocol-conformant outbound against fixed vectors; inbound construction remains `blocked-inbound-layout-ambiguity` until a current reference-router source pins the placement and Plan 112 holds it behind the typed `InboundBuildPendingReconciliation` gate), HKDF-SHA256 helper in `i2pr-crypto`, typed `ShortRequestRecord`/`ShortReplyRecord` encoders with `encode_with_rng`/`encode_deterministic_zero_padded` (`ShortBuildError::RandomnessUnavailable` fails closed), `decode` for postprocessor role recovery, `ShortBuildStateMachine` runtime-neutral state machine with `Prepared → Protecting → ReadyForDelivery → AwaitingReply → Established` (plus the bounded terminal failures) and the `ShortBuildPath::validate` direction/role topology validator that rejects non-final OBEP, OBEP on inbound paths, missing IBGW, and missing OBEP, success-only `ShortBuildRegistrar`, role-aware `MessageHopProcessor` that decodes the role from the authenticated plaintext, deterministic `DeterministicResponder` peer simulator, `ExploratoryPoolReplyPathProvider` adapter that implements `i2pr_netdb::ReplyPathProvider` and flips the Plan 106 NetDB seam from `BlockedExploratoryTunnelUnavailable` to `Available` when a real inbound tunnel is registered, frozen `fixed_vectors` module whose expected values were generated from an independent reference Noise-N + HKDF-SHA256 + ChaCha20-Poly1305 oracle and are committed as Rust constants, explicit `validate_count_prefixed_short_payload` / `encode_count_prefixed_short_payload` helpers that enforce the canonical STBM/OTBRM `count || records` contract (Plan 112 outbound pre-delivery closure), and the Rust-only reference provenance test under `crates/i2pr-tunnel/tests/plan111_reference_vectors.rs` that re-derives the same frozen bytes from a pure-Rust reference path; no live mixed-router tunnel execution, no transit participation | None |
...
source pins the placement.
Milestone 4A is now
`local-foundation-complete-short-build-outbound-conformant-fixed-vectors`
with `inbound_short_build = disabled-pending-layout-resolution`; the
