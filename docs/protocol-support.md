...
| SAM 3.1 independent-client closure (Plan 140) | Experimental; non-advertised; closure blocked | 7 | [`plans/140-m7-sam31-interoperability-closure.md`](../plans/140-m7-sam31-interoperability-closure.md), [`plans/140-status.md`](../plans/140-status.md), [`tests/integration/sam/README.md`](../tests/integration/sam/README.md) | No independent-client pass: pinned `i2plib` uses I2P `-~` Base64 for imported private destinations; pinned `txi2p` cannot load locally without legacy `ometa`; no live SAM STREAM raw handoff evidence | None; local Rust SAM protocol/session/forward/naming tests remain the CI-capable evidence lane |
