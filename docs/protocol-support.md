...
| SAM 3.1 independent-client closure (Plan 140 → Plan 142 corrective; Plan 143 / 144 still open) | Experimental; non-advertised | 7 | [`plans/140-m7-sam31-interoperability-closure.md`](../plans/140-m7-sam31-interoperability-closure.md), [`plans/140-status.md`](../plans/140-status.md), [`plans/142-m7-sam31-encoding-private-destination-corrective.md`](../plans/142-m7-sam31-encoding-private-destination-corrective.md), [`plans/142-status.md`](../plans/142-status.md), [`tests/integration/sam/README.md`](../tests/integration/sam/README.md) | **Plan 142 closed** the SAM Base64 / private-destination compatibility sub-claim: codec now uses I2P Base64 (`-`/`~`, `=` padding), independently corroborated against i2pd `libi2pd/Base.{h,cpp}`, Java I2P `PrivateKeyFile.java`, and i2plib `I2P_B64_CHARS = "-~"`. Plan 143 (same-socket live STREAM CONNECT/ACCEPT product bridge) and Plan 144 (two-independent-client final M7 closure) remain open; pinned `txi2p` still cannot load locally without legacy `ometa`. | None; local Rust SAM protocol/session/forward/naming tests plus the Plan 142 reference-vector set remain the CI-capable evidence lane |
