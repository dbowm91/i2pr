...
| I2CP | Experimental structural wire/profile foundation; non-advertised | 9 | `specs/protocols/10-i2cp-service-tunnels.md`, [`plans/163-m9-i2cp-roadmap.md`](../plans/163-m9-i2cp-roadmap.md), [`plans/164-m9-i2cp-protocol-and-wire-foundation.md`](../plans/164-m9-i2cp-protocol-and-wire-foundation.md), [`plans/164-status.md`](../plans/164-status.md) | Plan 164 `i2pr-api::i2cp`: `0x2a` preamble, strict common frame (64 KiB ceiling, incremental decoder), exact type IDs/directions/dispositions for all 25 assigned types, structural codecs for the M9 profile (GetDate/SetDate, session create/reconfigure/destroy/status, variable-lease request, Standard-LeaseSet2 publication with ordered redacted zeroized keys, send/expires, payload/status, bandwidth, destination/host lookup, disconnect), strict trailing-byte rejection, typed deprecated/unsupported/unknown classification, payload/gzip metadata contract with recorded expansion ceiling; committed `tests/fixtures/i2cp/` corpus with `scripts/check-i2cp-vectors.sh` and `crates/i2pr-api/tests/i2cp_vectors.rs`. No listener, session, destination activation, or client-interoperability claim; those belong to Plans 165–170. | None |
