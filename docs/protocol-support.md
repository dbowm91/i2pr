...
| SSU2 | Experimental v2 foundation; non-advertised | 8 | `specs/protocols/09-ssu2.md`, [`plans/155-m8-ssu2-v2-protocol-foundation-and-addresses.md`](../plans/155-m8-ssu2-v2-protocol-foundation-and-addresses.md), [`plans/155-status.md`](../plans/155-status.md) | Runtime-neutral `i2pr-transport-ssu2` (no Tokio, no sockets, no async): strict v2 RouterAddress parsing/validation (`SSU2`, `v=2`, 32-byte static/intro keys, numeric-IP host, UDP port, MTU floor 1280, bounded caps, bounded introducers; v3/v4 classified unsupported/deferred), structural long/short header codecs (types 0/1/2/6/7/9/10/11, version 2, network ID 2), and the bounded authenticated-plaintext block vocabulary with unknown-block byte/count ceilings. No handshake, no header protection/AEAD, no UDP socket, no interoperability claim; those belong to Plans 156–161. PQ-hybrid v3/v4 is deferred compatibility-watch debt; SSU1 remains unsupported. | `cargo test --locked -p i2pr-transport-ssu2 --all-targets` plus `bash scripts/check-ssu2-vectors.sh` |
