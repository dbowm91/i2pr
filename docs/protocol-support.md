...
| --- | --- | --- | --- | --- | --- |
| SAM 3.1 self-composed localhost STREAM product (Plan 149) | Experimental; non-advertised | 7 | [`plans/149-m7-sam31-self-composing-local-product-corrective.md`](../plans/149-m7-sam31-self-composing-local-product-corrective.md), [`plans/149-status.md`](../plans/149-status.md) | `SESSION CREATE` transactionally composes one shared `Arc<DestinationIdentity>`, signed local LeaseSet2, local-product delivery fabric, SAM bridge, stream ownership, and one supervised destination driver. The daemon resolves local peer LeaseSet2 records through its explicit directory, preserves same-read raw bytes, implements byte-exact CONNECT/ACCEPT `SILENT` behavior and authenticated non-silent ACCEPT peer metadata, records typed delivery degradation, and cleans up Streaming/SAM ownership on terminal CLOSE/RESET/cancellation. Canonical evidence is the black-box `crates/i2pr-daemon/tests/sam_stream_self_composed.rs` suite, which uses only listener TCP and SAM commands after startup. This localhost application-product seam does not claim live router interoperability; independent clients and final FORWARD/NAMING evidence remain Plan 150. | None |
...
| SAM 3.1 forward/naming hardening (Plan 139) | Experimental; non-advertised | 7 | [`plans/139-m7-sam31-forward-naming-hardening.md`](../plans/139-m7-sam31-forward-naming-hardening.md), [`plans/139-status.md`](../plans/139-status.md), `specs/protocols/08-sam.md` | Loopback-only, control-socket-owned STREAM FORWARD with atomic ACCEPT exclusion, bounded/cancellable raw bridge, session-scoped `NAME=ME`, strict canonical full-Destination lookup, locally-known b32 lookup, no DNS/address book, and explicit unsupported-feature replies; real-loopback tests only; independent-client interoperability deferred to Plan 140 | None |
| SAM 3.1 independent-client closure (Plan 140 → Plan 142 corrective; Plan 146 bidirectional reference requalification; Plan 147 closed; Plan 148 blocked on external client build) | Experimental; non-advertised | 7 | [`plans/140-m7-sam31-interoperability-closure.md`](../plans/140-m7-sam31-interoperability-closure.md), [`plans/140-status.md`](../plans/140-status.md), [`plans/142-m7-sam31-encoding-private-destination-corrective.md`](../plans/142-m7-sam31-encoding-private-destination-corrective.md), [`plans/142-status.md`](../plans/142-status.md), [`plans/146-m7-sam31-private-destination-reference-requalification.md`](../plans/146-m7-sam31-private-destination-reference-requalification.md), [`plans/146-status.md`](../plans/146-status.md), [`plans/147-m7-sam31-dedicated-raw-stream-driver-corrective.md`](../plans/147-m7-sam31-dedicated-raw-stream-driver-corrective.md), [`plans/147-status.md`](../plans/147-status.md), [`plans/148-m7-sam31-independent-client-final-closure.md`](../plans/148-m7-sam31-independent-client-final-closure.md), [`plans/148-status.md`](../plans/148-status.md), [`tests/integration/sam/README.md`](../tests/integration/sam/README.md) | **Plan 142 closed** the SAM Base64 / private-destination compatibility sub-claim: codec now uses I2P Base64 (`-`/`~`, `=` padding), independently corroborated against i2pd `libi2pd/Base.{h,cpp}`, Java I2P `PrivateKeyFile.java`, and i2plib `I2P_B64_CHARS = "-~"`. **Plan 146 closed** the bidirectional private-destination reference requalification against the pinned Java I2P 2.12.0 (`2800040deee9bb376567b671ef2e9c34cf3e30b6`) and i2pd 2.60.0 (`f618e417dbd0b7c5956af8f0d5a6b0ee78caf35e`) references — `crates/i2pr-daemon/tests/sam_plan146_reference.rs` proves both directions and the real-loopback `SESSION CREATE STYLE=STREAM DESTINATION=<reference-PRIV>` smoke (Plan 146 §10). Plan 146 also relaxed the reconstruction invariant: standard Java I2P `PrivateKeyFile` and i2pd `IdentityEx` populate the destination encryption public field with random bytes for destinations, so the new `DestinationIdentity::from_imported` constructor preserves the destination bytes verbatim and only enforces `signing_public == EdDSA(signing_seed)`. **Plan 147 closed** the dedicated same-socket raw TCP↔Streaming product bridge: `crates/i2pr-daemon/tests/sam_stream_raw_product.rs` is the canonical localhost byte-product lane, with bidirectional 1024/2048-byte application-byte exchange through the full Plan 129 destination/Streaming stack. **Plan 148 is `blocked-external-client-build-failure`** per [`plans/148-status.md`](../plans/148-status.md): the pinned i2plib (`6edf51cd5d21cc745aa7e23cb98c582144884fa8`) and libsam3 (`e0da4f4d8d3ca670fef86fd1046dab7c14afc5b7`) sources are not present in the local cache and no build/install lane exists for them; two-independent-client final closure plus FORWARD/naming byte-path revalidation remain blocked. No SAM feature is advertised. | None; local Rust SAM protocol/session/forward/naming tests plus the Plan 146 bidirectional reference evidence at `crates/i2pr-daemon/tests/sam_plan146_reference.rs` and the Plan 142 reference-vector set remain the CI-capable evidence lane |
