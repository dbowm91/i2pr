...
crates/
  ...
  i2pr-client/              Local destination identity, dedicated tunnel pools, signed Standard LeaseSet2 generation and lifecycle, bounded local payload contracts, router-local destination registry (Plan 120), ECIES-X25519-AEAD-Ratchet destination Garlic session layer (Plan 121), destination routing / LeaseSet2 NetDB composition (Plan 122), minimal I2P Streaming core with signed SYN/CLOSE/RESET, sequence/ACK/NACK, retransmit, and congestion (Plan 123)
...
Development targets a smaller interoperable-router milestone before the complete MVP. The current product milestone is **Milestone 6** (destinations, garlic, LeaseSet2, streaming), sequenced in [`plans/118-123-milestone6-router-construction-roadmap.md`](plans/118-123-milestone6-router-construction-roadmap.md). Plan 119 closed as `passed-leaseset2-protocol-foundation`; Plan 120 closed as `passed-destination-lifecycle-and-pools`; Plan 121 closed as `passed-ecies-destination-session-layer`; Plan 122 closed as `passed-local-destination-routing`; Plan 123 closed as `passed-minimal-streaming-core` with the strict I2P Streaming protocol surface, signed SYN / CLOSE / RESET handshakes, sequence/ACK/NACK, retransmit, congestion, send/receive windows, and a deterministic two-destination local trajectory. The next executable plan is **Plan 124** (streaming runtime adapter + UDP/TCP transport handoff).
