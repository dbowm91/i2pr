...
Development targets a smaller interoperable-router milestone before the complete MVP. The current product milestone is **Milestone 6** (destinations, garlic, LeaseSet2, streaming), sequenced in [`plans/118-123-milestone6-router-construction-roadmap.md`](plans/118-123-milestone6-router-construction-roadmap.md). Plan 119 closed as `passed-leaseset2-protocol-foundation`; Plan 120 closed as `passed-destination-lifecycle-and-pools`; Plan 121 closed as `passed-ecies-destination-session-layer`; Plan 122 closed as `passed-corrected-local-destination-routing` (Plan 124 corrective closure); Plan 123 remains `provisional-awaiting-plan125-correction` (Plan 125 owns the Streaming protocol-6 framing correction and round-trip trajectory). Plan 124 closed as `passed-plan122-corrective-closure` and lands the canonical ECIES Garlic carrier through the destination outbound tunnel. The next executable plan is **Plan 125** (Streaming protocol-6 framing correction + reply round-trip).
