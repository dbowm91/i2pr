...
The local Milestone 6 product (destinations, garlic, LeaseSet2, Streaming) remains closed under corrected local-correctness semantics via [**Plan 134**](plans/134-status.md). Independent-router interoperability is tracked as external acceptance debt. Milestone 7 / SAM closed the SAM 3.1 private-destination interoperability sub-claim via [**Plan 146**](plans/146-status.md) (`passed-m7-sam31-private-destination-reference-requalification`) and the dedicated raw STREAM socket driver via [**Plan 147**](plans/147-status.md) (`passed-m7-sam31-dedicated-raw-stream-driver`) — real localhost SAM TCP sockets now move bidirectional binary application bytes through the full local destination/Streaming product path under bounded backpressure and the Plan 129 delivery seam. The I2P Base64 correction from Plan 142 is retained, and the Plan 143/144 local delivery plus in-process handshake work is retained as regression coverage. The Milestone 7 external acceptance debt (two independent clients) remains **Plan 148**. [**Plan 145**](plans/145-status.md) remains the corrective authority, with execution sequence **Plan 146 (closed) → Plan 147 (closed) → Plan 148**. The `[sam]` config section remains disabled by default and loopback-only when enabled.
