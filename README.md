...
The local Milestone 6 product (destinations, garlic, LeaseSet2, Streaming) remains closed under corrected local-correctness semantics via [**Plan 134**](plans/134-status.md). Independent-router interoperability is tracked as external acceptance debt. Milestone 7 / SAM is **not yet closed**, but the SAM 3.1 private-destination interoperability sub-claim is now closed via [**Plan 146**](plans/146-status.md) (`passed-m7-sam31-private-destination-reference-requalification`) — bidirectional reference evidence against the pinned Java I2P 2.12.0 (`2800040deee9bb376567b671ef2e9c34cf3e30b6`) and i2pd 2.60.0 (`f618e417dbd0b7c5956af8f0d5a6b0ee78caf35e`) references proves the canonical SAM private-destination / PrivateKeyFile representation in both directions and through the real-loopback `SESSION CREATE` smoke. The I2P Base64 correction from Plan 142 is retained, and the Plan 143/144 local delivery plus in-process bidirectional Streaming handshake work is useful regression coverage. The remaining gaps prevent the Milestone 7 close: the daemon still lacks the dedicated TCP↔Streaming raw socket driver and bounded ACK/retransmit runtime ownership, and zero independent clients have yet moved application bytes through the real listener. [**Plan 145**](plans/145-status.md) is the current corrective authority, with execution sequence **Plan 146 (closed) → Plan 147 → Plan 148**. The `[sam]` config section remains disabled by default and loopback-only when enabled.
