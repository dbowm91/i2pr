...
Development targets a smaller interoperable-router milestone before the complete MVP. The current product milestone is **Milestone 6** (destinations, garlic, LeaseSet2, streaming), sequenced in [`plans/118-123-milestone6-router-construction-roadmap.md`](plans/118-123-milestone6-router-construction-roadmap.md). Plan 119 closed as `passed-leaseset2-protocol-foundation`; Plan 120 closed as `passed-destination-lifecycle-and-pools`; Plan 121 closed as `passed-ecies-destination-session-layer`; Plan 122 closed as `passed-corrected-local-destination-routing` (Plan 124 corrective closure); Plan 123 closed as `passed-corrected-minimal-streaming-local` (restored by Plan 125); Plan 124 closed as `passed-plan122-corrective-closure`; Plan 125 closed as `passed-milestone6-local-corrective-closure` and lands the canonical RFC 1952 gzip client payload wire format, the real Plan 125 §6/§7 SYN/SYN-response lifecycle, the `StreamingDestinationAdapter`, and the corrected `SystemClock`. Milestone 6 local product is now complete. The next executable product work is the SAM baseline planning (Milestone 7).
