...
The local Milestone 6 product (destinations, garlic, LeaseSet2, Streaming) remains closed under corrected local-correctness semantics via [**Plan 134**](plans/134-status.md). Independent-router interoperability is tracked as external acceptance debt. Milestone 7 / SAM is **not yet closed**: Plan 140 identified an incorrect SAM Base64 representation and an incomplete CONNECT/ACCEPT raw product path. [**Plan 141**](plans/141-status.md) is the current corrective authority, with Plan 142 next (I2P Base64/private-destination compatibility), followed by Plan 143 (live same-socket STREAM bridge) and Plan 144 (two independent SAM clients and final M7 closure). The `[sam]` config section remains disabled by default and loopback-only when enabled.
