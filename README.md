...
The local Milestone 6 product (destinations, garlic, LeaseSet2, Streaming) remains closed under corrected local-correctness semantics via [**Plan 134**](plans/134-status.md). Independent-router interoperability is tracked as external acceptance debt. Milestone 7 / SAM is **partially closed**: the SAM Base64 alphabet and the private-destination compatibility sub-claim are now closed via [**Plan 142**](plans/142-status.md), with the codec using I2P Base64 (`-`/`~`, `=` padding) and three independent reference vectors (i2pd `libi2pd/Base.{h,cpp}`, Java I2P `PrivateKeyFile.java`, i2plib `I2P_B64_CHARS = "-~"`). The same-socket STREAM CONNECT/ACCEPT product bridge (the captured-outbound test seam removed; Plan 129 destination stack drives the live bridge through the runtime-neutral `i2pr_client::deliver` seam) is closed via [**Plan 143**](plans/143-status.md) with `crates/i2pr-daemon/tests/sam_stream_product.rs` as the canonical product evidence. The full bidirectional in-process handshake (the Plan 143 mirror/canonical-streaming routing asymmetry fixed) is closed via [**Plan 144**](plans/144-status.md), with `crates/i2pr-daemon/tests/sam_stream_independent.rs` as the canonical in-process evidence. [**Plan 141**](plans/141-status.md) remains the broader corrective authority; Plan 144 records the per-stream TCP↔Streaming raw byte bridge and two-independent-client evidence lane as deferred follow-up work. The `[sam]` config section remains disabled by default and loopback-only when enabled.
