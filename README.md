...
Plan 130 status: **`passed-milestone6-final-wire-runtime-corrective-closure`** ([`plans/130-status.md`](plans/130-status.md)). Plan 130 is the single remaining Milestone 6 implementation pass and supersedes the Plan 129 gate decision; it repairs the four narrow defects the post-closure audit found while retaining the Plan 129 integrated topology and the successful Plans 126-128 surfaces. (1) Production ECIES ephemerals now use reference-compatible randomized Elligator2 representations — `EciesEphemeralKeypair::generate` draws the two normative CSPRNG high bits (`encodedKey[31] |= random & 0xc0`) while `from_seed_bytes` stays the deterministic vector constructor and every frozen Plan 126 KDF/Noise vector reproduces byte-for-byte; independent pure-Python fixtures pin all four high-bit variants and both Java/i2pd encode branches decoding to one X25519 public key ([`specs/references/elligator2-production-representation.md`](specs/references/elligator2-production-representation.md); no hand-written Elligator arithmetic). (2) Streaming application sequence numbering starts at 1 post-SYN: SYN and SYN response own sequence 0, a seq-0 non-SYN packet is the plain-ACK control form that never enters the receive window or schedules an acknowledgement. (3) ACK state is semantic: `ackThrough == 0` validly acknowledges the handshake slot unless NO_ACK is set; cumulative ACKs are NACK-aware (explicitly NACKed packets stay tracked per the Java `Connection.ackPackets` contract), receiver ACK/NACK views follow `MessageInputStream.updateAcks` (ackThrough = highest received incl. buffered out-of-order packets with bounded missing-sequence NACKs), and a synchronous `poll_acks(now_ms)` emits coalesced standalone delayed ACKs on the 750 ms reference default with piggyback suppression and no ACK-of-ACK loop. (4) The wire `destination_port` owns inbound dispatch: the adapter lost its caller-supplied listener override, listener matching is exact-port → wildcard port-0 → typed `NoMatchingListener`, established connections enforce their port tuple (typed `PortTupleMismatch`), and tunnel duplicate windows persist across fixture deliveries with exact replays failing as typed `DuplicateCell`. Eleven full-stack corrective trajectories live in `crates/i2pr-client/tests/plan130_trajectory.rs`. Milestone 6 local product is **passed** under the corrected semantics; interoperability remains unclaimed.
