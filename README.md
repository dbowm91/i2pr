...
Development targets a smaller interoperable-router milestone before the complete MVP. The current product milestone is **Milestone 6** (destinations, garlic, LeaseSet2, streaming), sequenced in [`plans/118-123-milestone6-router-construction-roadmap.md`](plans/118-123-milestone6-router-construction-roadmap.md) and its corrective roadmap [`plans/126-129-milestone6-final-corrective-roadmap.md`](plans/126-129-milestone6-final-corrective-roadmap.md). Plan 119 closed as `passed-leaseset2-protocol-foundation`; Plan 120 closed as `passed-destination-lifecycle-and-pools`; Plan 121 closed as `passed-ecies-destination-session-layer` (superseded dialect corrected by Plan 126, final local closure restored by Plan 127); Plan 122 closed as `passed-corrected-local-destination-routing` (Plan 124 corrective closure); Plan 123 closed as `passed-corrected-streaming-wire-local` (restored by Plans 128/129); Plan 124 closed as `passed-corrected-destination-routing-local-closure`; Plan 125 is `superseded-by-final-corrective-closure`; Plan 126 closed as `passed-ecies-destination-ratchet-corrective-foundation`; Plan 127 closed as **`passed-destination-session-routing-final-closure`**; Plan 128 closed as **`passed-streaming-wire-protocol-corrective-closure`** (restoring `plan_123 = passed-corrected-streaming-wire-local`). Plan 129 is **`superseded-by-plan130-final-gate`**; Plan 130 is **`superseded-by-plan131-final-local-correctness-gate`** (historical evidence retained in [`plans/130-status.md`](plans/130-status.md)); Plan 131 closed as **`passed-milestone6-final-local-correctness-closure`** ([`plans/131-status.md`](plans/131-status.md)) under the corrective roadmap [`plans/126-130-milestone6-final-corrective-roadmap.md`](plans/126-130-milestone6-final-corrective-roadmap.md) extended by the Plan 131 plan-of-record ([`plans/131-m6-final-local-correctness-closure.md`](plans/131-m6-final-local-correctness-closure.md)): production Elligator branch randomization via the reviewed `elligator2 = 0.1.0` primitive (`curve25519-elligator2 = 0.1.0-alpha.2` retired because its `RFC9380` branch choice was deterministic rather than caller-randomizable and its `Randomized` mode rotated the derived X25519 public key), three-layer replay separation (tunnel duplicate window, consumed ECIES session tag, fresh-ECIES-reseal Streaming sequence), connection-owned I2P port tuple asserted on every established `send_data` / `send_close` / `send_reset`, source-port `0` as valid I2P "unspecified", and side-effect-free oversized `send_data` rollback. Seven deterministic Plan 131 trajectories live in `crates/i2pr-client/tests/plan131_trajectory.rs`. Plan 132 closed as **`passed-milestone6-final-evidence-and-transactional-closure`** ([`plans/132-status.md`](plans/132-status.md)): strict Elligator2 receive-domain validation masking high bits and rejecting `r >= 2^254 - 10` via the `is_canonical_elligator_representative` helper before delegating to `elligator2::from_representative`; three independent layer-isolated replay trajectories (persistent tunnel `DuplicateToken`, consumed ECIES session tag, fresh-ECIES-reseal Streaming sequence) with an artifact-preserving test seam in `crates/i2pr-client/tests/plan132_trajectory.rs`; and `&mut self` transactional send ordering on `send_data` / `send_close` / `send_reset` (Phase 1 immutable validation → Phase 2 fallible wire construction → Phase 3 single commit point with `assert_eq!(sequence, planned_sequence)` guarantee). Milestone 6 local product is **passed** under the corrected local-correctness semantics (`milestone6_interoperable = not-yet-claimed`; external acceptance debt retained separately). Corrective Milestone 6 planning stops here; the next product layer is **SAM baseline planning (Milestone 7)**.
