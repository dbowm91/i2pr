...
The local Milestone 6 product (destinations, garlic, LeaseSet2, Streaming) is closed under corrected local-correctness semantics via [**Plan 134**](plans/134-status.md). Independent-router interoperability is tracked as external acceptance debt. **Plan 139 (SAM 3.1 STREAM FORWARD and naming hardening)** extends the Plan 138 loopback listener with loopback-only, ownership-bound forwarding, bounded raw bridging, session-scoped `NAME=ME`, canonical public-Destination lookup, locally-known `.b32.i2p` lookup, and explicit unsupported-feature replies. Plan 140 audited the independent-client closure and is currently blocked: the live STREAM raw handoff and external private-destination compatibility still need correction. The `[sam]` config section defaults to `enabled = false` so the production daemon never binds. See [the Plan 140 status](plans/140-status.md) and the [localhost interoperability lane](tests/integration/sam/README.md).
