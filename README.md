...
- The Plan 199 public-client corrective uses public `I2PClient`/`I2PSession` and `I2PSocketManager` APIs, with two disposable Java routers exchanging ordinary RouterInfo first. The bootstrap row passes; Plan 200 now proves Router A/B main-NetDB bootstrap and the helper READY semantics. Plan 201 owns the smallest standards-compatible corrective once the Plan 200 classification is recorded. Mandatory destination and Streaming rows remain blocked with fail-closed provenance.
