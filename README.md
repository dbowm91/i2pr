...
m6_java_mixed_router_interop = passed-via-plan194-with-bounded-sam-ls2-publication-gap (full §5.1-§5.5 destination + streaming layers lifted into the Java second-family lane; 26 passed + 22 blocked with `plan194-java-stop` provenance + 0 failed against the exact-pinned Java I2P 2.13.0 cache)
...
The Milestone 6 mixed-router interoperability qualification is **closed for both families via [Plan 193](plans/193-status.md) + [Plan 194](plans/194-status.md)** (experimental, loopback-only):
...
- The Java I2P 2.13.0 family proves the bounded destination + streaming matrix under the documented **SAM-bridge LS2-publication gap**: Java's SAM bridge does NOT auto-publish the SAM-destination LS2 to the local NetDB in a controlled private topology where the reference has no peer tunnels to build a client tunnel for the lease; i2pd's SAM bridge does publish immediately. The gap is a Java-internal architectural fact, not an i2pr regression. Fresh external `bash tests/integration/m6-interop/run-java.sh` against the exact-pinned Java I2P 2.13.0 (`9134f808337b401e8e53c73734c81fab04280c9d`) cache proves 26 rows passed (local + every Plan 196 topology row + authenticated-session + SAM-destination-creation + tunnel-build-install + manager-cleanup) + 22 rows blocked with `plan194-java-stop` provenance (LS2-lookup-dependent + delivery-dependent + streaming-layer rows) + 0 failed rows.

Static checkers `scripts/check-streaming-tunnel-evidence.sh` (33 guarded labels, both i2pd + Java harnesses wired) and `scripts/check-destination-tunnel-evidence.sh` (21 guarded labels, both i2pd + Java harnesses wired) and `scripts/check-m6-mixed-router-acceptance-evidence.sh` (11 guarded labels, both pins referenced) are wired into the routine Linux CI floor alongside the Plan 196 controlled-topology invariants and the Plan 197 §9 PQ parser tolerance invariants. Plan 194's bounded two-family claim is `milestone6_interoperable = passed-via-plan193+194-with-bounded-sam-ls2-publication-gap`. Plan 195 (M10 remote service interop) is unblocked:
