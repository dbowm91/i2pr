...
pub struct DestinationTunnelCounters {
    ...
    /// Plan 219 §6 typed fact — Router B's live RouterInfo, as
    /// currently published by Router B itself, advertises the
    /// `f` floodfill capability (`true`). `false` is itself a
    /// typed observation, not a default; J219-A classification
    /// consumes this counter.
    pub j219_b_live_ri_has_f: bool,
    /// Plan 219 §6 typed fact — Router A's authoritative store
    /// holds Router B's current RouterInfo and that record
    /// advertises the `f` capability.
    pub j219_a_stored_b_ri_has_f: bool,
    /// Plan 219 §6 typed fact — Router A's PeerManager
    /// capability index lists Router B's hash under capability
    /// `f`.
    pub j219_a_peermanager_b_indexed_f: bool,
    /// Plan 219 §6 typed fact — `FloodfillPeerSelector` initial
    /// set cardinality observed at the helper reverse-send
    /// boundary; zero is itself a typed observation.
    pub j219_a_selector_input_count: u64,
    /// Plan 219 §6 typed fact — `FloodfillPeerSelector` selected
    /// set cardinality after profile/ranking; zero is the J219-D
    /// trigger.
    pub j219_a_selector_result_count: u64,
    /// Plan 219 §6 typed fact — Router A's main NetDB
    /// `getAllRouters()` cardinality reachable to the helper
    /// client-specific lookup, computed from
    /// `peerManager().getPeersByCapability('f')` plus the
    /// arbitrary-known-routers fallback in
    /// `IterativeSearchJob.runJob()`.
    pub j219_client_db_main_router_count: u64,
    /// Plan 219 §6 typed fact — helper client-specific NetDB
    /// `lookupLeaseSet` started.
    pub j219_client_db_lookup_started: bool,
    /// Plan 219 §6 typed fact — helper client-specific NetDB
    /// lookup peer selected.
    pub j219_client_db_lookup_peer_selected: bool,
    /// Plan 219 §6 typed fact — helper client-specific NetDB
    /// lookup result (`true` = succeeded with a LeaseSet
    /// returned; `false` = lookup failed or returned no LS).
    pub j219_client_db_lookup_result: bool,
    /// Plan 219 §6 typed fact — OCMOSJ lease selection
    /// succeeded.
    pub j219_ocmosj_lease_selected: bool,
    /// Plan 219 §6 typed fact — OCMOSJ outbound client tunnel
    /// selection succeeded.
    pub j219_ocmosj_outbound_tunnel_selected: bool,
    /// Plan 219 §6 typed fact — OCMOSJ tunnel dispatch
    /// submitted (TunnelData emitted through the helper's
    /// outbound tunnel endpoint).
    pub j219_ocmosj_dispatch_submitted: bool,
}

/// Bounded, sanitized set of Plan 219 §6 typed facts the
/// destination driver collects from the controlled Java reference
/// to derive a single `J219-{A..J}` terminal classification.
///
/// Every field is bounded, privacy-safe, and never carries key
/// material or payload bytes. The struct is publicly
/// constructable from a typed evidence reader so the destination
/// driver can call
/// [`derive_j219_terminal_classification`](Self::derive_j219_terminal_classification)
/// after the harness has populated it.
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub struct J219TypedFacts {
    /// Router B's live RouterInfo `f` capability (observed from
    /// the controlled Java reference's own published RI).
    pub b_live_ri_has_f: bool,
    /// Router A's authoritative store holds Router B's current
    /// RouterInfo and that record advertises `f`.
    pub a_stored_b_ri_has_f: bool,
    /// Router A's PeerManager indexes Router B under capability
    /// `f`.
    pub a_peermanager_b_indexed_f: bool,
    /// `FloodfillPeerSelector` initial input cardinality.
    pub a_selector_input_count: u64,
    /// `FloodfillPeerSelector` post-ranking result cardinality.
    pub a_selector_result_count: u64,
    /// Helper-visible main-NetDB router cardinality reachable
    /// to the helper client-specific lookup.
    pub client_db_main_router_count: u64,
    /// Helper client-specific lookup started.
    pub client_db_lookup_started: bool,
    /// Helper client-specific lookup peer selected.
    pub client_db_lookup_peer_selected: bool,
    /// Helper client-specific lookup result (`true` = succeeded).
    pub client_db_lookup_result: bool,
    /// Helper OCMOSJ lease selected.
    pub ocmosj_lease_selected: bool,
    /// Helper OCMOSJ outbound client tunnel selected.
    pub ocmosj_outbound_tunnel_selected: bool,
    /// Helper OCMOSJ tunnel dispatch submitted.
    pub ocmosj_dispatch_submitted: bool,
    /// Java dispatch observed in the i2pr-side inbound pump.
    pub java_dispatch_observed: bool,
    /// i2pr inbound TunnelData delivered for the helper reply.
    pub i2pr_inbound_observed: bool,
}

/// Bounded terminal classification for the Plan 219
/// reverse-delivery investigation. The earliest failed boundary
/// (per Plan 219 §7) wins; `J219-J` is the qualified "reverse
/// delivery passed" terminal. No catch-all exists.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum J219Terminal {
    /// Router B's live RouterInfo lacks `f`.
    J219ABLiveRiNotF,
    /// Router B's RouterInfo as stored by Router A lacks `f`.
    J219BAStoredBRiNotF,
    /// Router A's PeerManager indexes Router B but not under `f`.
    J219CAPeerManagerMissingBF,
    /// Router A's `FloodfillPeerSelector` excludes Router B.
    J219DAFloodfillSelectorExcludesB,
    /// Helper client-specific NetDB lookup has no peer.
    J219EClientDbLookupHasNoPeer,
    /// Helper client-specific NetDB lookup sent no LS.
    J219FClientDbLookupSentNoLs,
    /// LeaseSet found but no outbound client tunnel.
    J219GLsFoundButNoOutboundClientTunnel,
    /// Java dispatch not observed.
    J219HJavaDispatchNotObserved,
    /// Java dispatch proven; i2pr inbound not observed.
    J219IJavaDispatchProvenI2prInboundNotObserved,
    /// Reverse delivery passed (qualified terminal).
    J219JReverseDeliveryPassed,
}

impl J219Terminal {
    /// Returns the canonical token string used in the
    /// `j219-classification` evidence row.
    pub fn token(self) -> &'static str {
        match self {
            Self::J219ABLiveRiNotF => "J219-A-B-LIVE-RI-NOT-F",
            Self::J219BAStoredBRiNotF => "J219-B-A-STORED-B-RI-NOT-F",
            Self::J219CAPeerManagerMissingBF => "J219-C-A-PEERMANAGER-MISSING-B-F",
            Self::J219DAFloodfillSelectorExcludesB => "J219-D-A-FLOODFILL-SELECTOR-EXCLUDES-B",
            Self::J219EClientDbLookupHasNoPeer => "J219-E-CLIENT-DB-LOOKUP-HAS-NO-PEER",
            Self::J219FClientDbLookupSentNoLs => "J219-F-CLIENT-DB-LOOKUP-SENT-NO-LS",
            Self::J219GLsFoundButNoOutboundClientTunnel => {
                "J219-G-LS-FOUND-BUT-NO-OUTBOUND-CLIENT-TUNNEL"
            }
            Self::J219HJavaDispatchNotObserved => "J219-H-JAVA-DISPATCH-NOT-OBSERVED",
            Self::J219IJavaDispatchProvenI2prInboundNotObserved => {
                "J219-I-JAVA-DISPATCH-PROVEN-I2PR-INBOUND-NOT-OBSERVED"
            }
            Self::J219JReverseDeliveryPassed => "J219-J-REVERSE-DELIVERY-PASSED",
        }
    }
}

impl J219TypedFacts {
    /// Constructs a `J219TypedFacts` whose 12 mandatory Plan 219 §6
    /// typed facts are populated. The two helper-side observed
    /// booleans (Java dispatch + i2pr inbound observed) are
    /// passed separately so the destination driver can derive
    /// them from the existing Plan 194/192 evidence keys without
    /// widening this struct's contract.
    #[allow(clippy::too_many_arguments)]
    pub const fn new(
        b_live_ri_has_f: bool,
        a_stored_b_ri_has_f: bool,
        a_peermanager_b_indexed_f: bool,
        a_selector_input_count: u64,
        a_selector_result_count: u64,
        client_db_main_router_count: u64,
        client_db_lookup_started: bool,
        client_db_lookup_peer_selected: bool,
        client_db_lookup_result: bool,
        ocmosj_lease_selected: bool,
        ocmosj_outbound_tunnel_selected: bool,
        ocmosj_dispatch_submitted: bool,
    ...
    /// Derives exactly one Plan 219 §7 terminal class. The
    /// earliest failed boundary wins; if every typed fact is in
    /// its passing state AND `i2pr_inbound_observed` is true,
    /// the qualified `J219-J` is returned. There is no catch-all;
    /// a fact that is `false` produces the corresponding
    /// terminal class.
    pub fn derive_j219_terminal_classification(&self) -> J219Terminal {
        if !self.b_live_ri_has_f {
            return J219Terminal::J219ABLiveRiNotF;
        }
        if !self.a_stored_b_ri_has_f {
            return J219Terminal::J219BAStoredBRiNotF;
        }
        if !self.a_peermanager_b_indexed_f {
            return J219Terminal::J219CAPeerManagerMissingBF;
        }
        if self.a_selector_input_count > 0 && self.a_selector_result_count == 0 {
            return J219Terminal::J219DAFloodfillSelectorExcludesB;
        }
        if self.client_db_lookup_started && !self.client_db_lookup_peer_selected {
            return J219Terminal::J219EClientDbLookupHasNoPeer;
        }
        if self.client_db_lookup_peer_selected && !self.client_db_lookup_result {
            return J219Terminal::J219FClientDbLookupSentNoLs;
        }
        if self.client_db_lookup_result && !self.ocmosj_lease_selected {
            return J219Terminal::J219GLsFoundButNoOutboundClientTunnel;
        }
        // OCMOSJ lease + outbound tunnel both present, dispatch
        // hidden -> J219-H; dispatch visible, i2pr inbound
        // missing -> J219-I; everything passes -> J219-J.
        if !self.ocmosj_dispatch_submitted {
            return J219Terminal::J219HJavaDispatchNotObserved;
        }
        if !self.java_dispatch_observed {
            return J219Terminal::J219HJavaDispatchNotObserved;
        }
        if !self.i2pr_inbound_observed {
            return J219Terminal::J219IJavaDispatchProvenI2prInboundNotObserved;
        }
        J219Terminal::J219JReverseDeliveryPassed
    }

    /// Returns the bounded Plan 219 §11 row emitted as the
    /// `j219-classification` evidence key value. The token is
    /// the canonical J219-{A..J} string and a defensive
    /// explanation of the earliest failed boundary, suitable for
    /// pastability into the closure record.
    pub fn classification_detail(&self) -> String {
        let terminal = self.derive_j219_terminal_classification();
        format!(
            "{} b_live_ri_has_f={} a_stored_b_ri_has_f={} \
             a_peermanager_b_indexed_f={} selector_input={} selector_result={} \
             client_db_main_router_count={} client_db_lookup_started={} \
             client_db_lookup_peer_selected={} client_db_lookup_result={} \
             ocmosj_lease_selected={} ocmosj_outbound_tunnel_selected={} \
             ocmosj_dispatch_submitted={} java_dispatch_observed={} \
             i2pr_inbound_observed={}",
            terminal.token(),
            self.b_live_ri_has_f,
            self.a_stored_b_ri_has_f,
            self.a_peermanager_b_indexed_f,
            self.a_selector_input_count,
            self.a_selector_result_count,
            self.client_db_main_router_count,
            self.client_db_lookup_started,
            self.client_db_lookup_peer_selected,
            self.client_db_lookup_result,
            self.ocmosj_lease_selected,
            self.ocmosj_outbound_tunnel_selected,
            self.ocmosj_dispatch_submitted,
            self.java_dispatch_observed,
            self.i2pr_inbound_observed,
        )
    }
}
...
impl DestinationTunnelCoordinator {
    ...
    /// Plan 219 §6 typed-fact writer.
    ///
    /// The Plan 219 investigation replaces coarse
    /// `router.config` / unscoped-grep evidence with a typed
    /// surface the destination driver can populate from a
    /// bounded, sanitized test-only read of the controlled Java
    /// reference's `RouterContext` accessors
    /// (`netDb().lookupRouterInfoLocally(...)`,
    /// `peerManager().getPeersByCapability('f')`, and
    /// `netDb().getAllRouters()`) plus the existing
    /// helper-side dispatcher evidence keys. Each documented
    /// `(label, value)` pair advances exactly one Plan 219
    /// counter; `value` MUST be a bounded sanitized string the
    /// static checker recognizes (`present`, `absent`,
    /// `count=N`, `lookup-started`, `lookup-peer-selected`,
    /// `lookup-succeeded`, `lookup-failed`, `lease-selected`,
    /// `tunnel-selected`, `dispatch-submitted`, `dispatch-not-submitted`).
    ///
    /// Returns `true` when the (label, value) pair is in the
    /// documented set; `false` otherwise (the observation is
    /// silently ignored so a future expansion of the documented
    /// set must update the static checker as well).
    pub fn note_j219_typed_fact(&mut self, label: &str, value: &str) -> bool {
        match (label, value) {
            ("b-live-ri-has-f", "present") => {
                self.counters.j219_b_live_ri_has_f = true;
                true
            }
            ("b-live-ri-has-f", "absent") => {
                // Counter stays false; record the observed
                // "false" so a static checker can reject the
                // absence as a missing-fact.
                true
            }
            ("a-stored-b-ri-has-f", "present") => {
                self.counters.j219_a_stored_b_ri_has_f = true;
                true
            }
            ("a-stored-b-ri-has-f", "absent") => true,
            ("a-peermanager-b-indexed-f", "present") => {
                self.counters.j219_a_peermanager_b_indexed_f = true;
                true
            }
            ("a-peermanager-b-indexed-f", "absent") => true,
            ("a-selector-input", "count") => {
                self.counters.j219_a_selector_input_count =
                    self.counters.j219_a_selector_input_count.saturating_add(1);
                true
            }
            ("a-selector-result", "count") => {
                self.counters.j219_a_selector_result_count =
                    self.counters.j219_a_selector_result_count.saturating_add(1);
                true
            }
            ("client-db-main-router-count", "count") => {
                self.counters.j219_client_db_main_router_count = self
                    .counters
                    .j219_client_db_main_router_count
                    .saturating_add(1);
                true
            }
            ("client-db-lookup-started", "lookup-started") => {
                self.counters.j219_client_db_lookup_started = true;
                true
            }
            ("client-db-lookup-started", "lookup-not-started") => true,
            ("client-db-lookup-peer-selected", "lookup-peer-selected") => {
                self.counters.j219_client_db_lookup_peer_selected = true;
                true
            }
            ("client-db-lookup-peer-selected", "lookup-peer-not-selected") => true,
            ("client-db-lookup-result", "lookup-succeeded") => {
                self.counters.j219_client_db_lookup_result = true;
                true
            }
            ("client-db-lookup-result", "lookup-failed") => true,
            ("ocmosj-lease-selected", "lease-selected") => {
                self.counters.j219_ocmosj_lease_selected = true;
                true
            }
            ("ocmosj-lease-selected", "lease-not-selected") => true,
            ("ocmosj-outbound-tunnel-selected", "tunnel-selected") => {
                self.counters.j219_ocmosj_outbound_tunnel_selected = true;
                true
            }
            ("ocmosj-outbound-tunnel-selected", "tunnel-not-selected") => true,
            ("ocmosj-dispatch-submitted", "dispatch-submitted") => {
                self.counters.j219_ocmosj_dispatch_submitted = true;
                true
            }
            ("ocmosj-dispatch-submitted", "dispatch-not-submitted") => true,
            _ => false,
        }
    }
