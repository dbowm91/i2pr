...
// Plan 220 D220-9 historical note: the Plan 219 typed-fact
// surface (per-stage counters, the aggregator struct, the
// terminal enum, and the typed-fact writer) lived here only for
// external interoperability classification and had no production
// runtime consumer, so it moved out of production source. The
// corrected P220 tri-state classifier lives in the external test
// driver (`crates/i2pr-daemon/tests/java_tunnel_external.rs`).
// Plan 201 §G counters above retain independent runtime value.
