...
Plan 189 = registered M6 Java second-family qualification and mixed-router closure (blocked-by-plan188-plan190-plan191; §8 cross-family ledger/checker/workflow landed, no second-family Java row yet)
...
Plan 191 = registered M6 inbound destination delivery boundary (Plan 190 §6 stop boundary E; next executable; inbound-delivery + destination-side ordering rows)
...
M6 destination remote interop = installs-proven-lookup-publication-outbound-passed-inbound-delivery-pending-plan191
...
M6 inbound destination delivery = blocked-pending-plan191 (Plan 190 §6 stop boundary E)
M6 mixed-router cross-family ledger = landed-via-plan189 (i2pd-only-runs; java-second-family-deferred-until-plan188-plan191-closes)
...
# only when a real tunneled lookup response arrives within the bounded wait
...
reply-path tunnel-ID corrective (see `plans/190-status.md`):
typed `InboundGatewayRoute` metadata in `i2pr-tunnel::DataPlaneRegistry`,
daemon-owned `reply_path_for_inbound_route` adapter,
`destination_tunnel_unit` regression rows with unequal IDs
(`0x9601` vs `0x9602`) round-tripping through the I2NP codec
with the gateway tuple on the wire, and lifecycle removal
cleaning the typed route atomically. Local Plan 187/188 suites
remain green (`destination_tunnel_unit` 31 passed,
`destination_tunnel_live` 9 passed, `exploratory_build_live`
11 passed). A fresh exact-pinned i2pd 2.61.0 external destination
lane proves the corrected reply path end-to-end: 3 destination
rows (`external-lease-lookup-tunnel`, `external-ls2-publication-tunnel`,
`external-destination-outbound`) flip from `blocked` to `passed`,
all Plan 187/188 `passed` rows remain `passed`. The corrected
lane stops at Plan 190 §6 boundary E: the destination message
reaches i2pd but the i2pd SAM bridge never observes a
`DATAGRAM RECEIVED` line, so `external-reference-received` and
...
layer plus the destination-side ordering rows. Plan 188 keeps
the `installed_ob/installed_ib` retention-passed status and
the corrected reply-path rows retained-passed via Plan 190;
its remaining destination rows are now blocked on Plan 191.
Plan 189 stays blocked until Plan 188 + Plan 191 + the deferred
