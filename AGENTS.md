...
Plan 202 = passed-m10-production-remote-destination-and-streaming-composition
   Plan 203 = retained-partial-m10-positive-remote-http-and-irc-application-interop-scaffold-superseded-by-plan207 (the `m10_positive_remote_http_and_irc_application_interop` driver is retained as a historical scaffold; its `record_remote_application_observation` label-injection pattern is no longer accepted as a counted proof; the manager's free observation helper remains for backwards compatibility but new code must depend on the documented Plan 207 / Plan 209 subfact rows for counted evidence)
   Plan 195 = evidence-passed-m10-remote-independent-service-via-plan208-and-plan209-pending-plan204-normalization (Plan 195 absorbed the Plan 207 + Plan 209 positive application evidence; reactivated from the historical `registered-blocked-by-plan199` interpretation by the Plan 204 docs/CI normalization pass)
    Plan 204 = in-progress-docs-and-authority-normalization-blocked-on-plan205-sam-bridge-pivot (intentionally narrow docs/CI/evidence-authority normalization pass on top of Plans 200/202/206/207/208/209; the M6/M10 closed authority transitions in §7/§12 of `plans/204-m10-final-closure-evidence-authority-and-documentation-normalization.md` stay deferred until Plan 205's SAM-bridge helper pivot closes Plan 201's Java mandatory rows; no product bug hidden, no synthetic `passed` evidence introduced)
...
m10_remote_transport_core = passed-via-plan212-and-plan213 (Plan 213 closed the generic router-backed Direction A+B external proof twice on exact commit ef59fb3: hosted router-generic runs 35169304975 + 35169987455, both P213-N-passed; routine CI run 35169271477 green on the same SHA)
     m10_remote_application_interop = passed-hosted-double-pass-on-a71e0193 (Plan 215 re-closure double-pass on the dependabot-merged head `a71e0193c420c8d8464fd05ff67d5323515ed4dd` — run `35347780246` + run `35349313549` — both observed `P213-N-passed` + `P214-N-passed` with 73/73 rows green including the `plan214-clean-resource-baseline` and `plan214-cleanup-kills-on-timeout` rows; the original Plan 215 closure double-pass on `1992d67ffe1d37c1d5c225fff494c5bf02ba00b3` — run `35309158441` + run `35309655867` — is retained as the prior provenance)
     ...
     Plan 213 = passed-m10-router-backed-generic-external-qualification (see plans/213-status.md)
      Plan 214 = passed-m10-product-only-remote-http-and-irc-application-closure (see plans/214-status.md; the Plan 215 hosted double-pass on `1992d67` flipped the authority — run `35309158441` + run `35309655867` both observed `P214-N-passed` with 73/73 rows green including the `plan214-clean-resource-baseline` and `plan214-cleanup-kills-on-timeout` rows that the Plan 215 §5 cleanup hardening introduced; the Plan 215 re-closure double-pass on `a71e0193` — run `35347780246` + run `35349313549` — re-proves the gate on the dependabot-merged head with the same `P214-N-passed` outcome)
      Plan 215 = passed-m10-hosted-plan214-tunnel-config-generation-corrective-and-exact-head-reverification (see plans/215-status.md; the source-side corrective landed as Commit `0fbacc3` (deterministic `write_plan214_tunnels_conf` + `validate_plan214_tunnels_conf` + `plan214-reference-tunnel-config-sanity` evidence row + `test-plan215-tunnels-conf.sh` + §29 static checker), the §5 cleanup hardening landed as Commit `1992d67` (`setsid` on the loopback fixtures + SIGKILL in `stop_group` + bounded grace window with KILL fallback + scoped pgrep patterns matching this run's `--facts` / `--datadir` markers), the initial hosted double-pass landed on `1992d67` as runs `35309158441` + `35309655867` — both `P214-N-passed`, and the re-closure hosted double-pass landed on the dependabot-merged head `a71e0193` as runs `35347780246` + `35349313549` — both `P214-N-passed`; no product code change in either pass)
     next_m10_application_plan = plan215-reclosure-passed-on-a71e0193 (the Plan 215 re-closure double-pass is closed on the dependabot-merged immutable SHA `a71e0193` — run `35347780246` + run `35349313549` both observed `P213-N-passed` + `P214-N-passed`; the original Plan 215 closure double-pass on `1992d67` — run `35309158441` + run `35309655867` — is the prior provenance; the remaining M6 Java second-family row stays blocked on the Plan 201 terminal `P200-{A..H}` classification and the Plan 204 docs/CI normalization pass owns the final cross-milestone authority convergence)
...
invariants for the M10 real service-Destination network material
and inbound Streaming corrective: `SHA256(reference.router_info_bytes)`
is rejected as a service LeaseSet lookup target; the counted
remote compose path is forbidden from calling
`dummy_outbound_tunnel()`; the inbound tunnel owner reverse map
must exist (`register_inbound_tunnel_owner` /
`unregister_inbound_tunnel_owner` / `inbound_tunnel_owner` /
`inbound_tunnel_owner_pairs`); recovered Garlic envelopes must
dispatch through the canonical
`DestinationDispatcher::dispatch_garlic_envelope` instead of
being silently dropped; the `SamDestinationBridge::compose_adapter_send_owned_fields`
helper must be the placeholder-free entry point the
production `compose_remote_cells` delegates to; and at least one
`plan210_…` unit row must be discoverable in
`crates/i2pr-daemon/src/service_tunnels.rs`. The eight
`plan210_real_service_destination_material_tests` rows lock the
...
bounded `tests/integration/service-tunnels/clients/parse_i2pd_destination.py`
...
Current handoff: **Plan 215 re-closure double-pass landed on the
dependabot-merged head `a71e0193c420c8d8464fd05ff67d5323515ed4dd`
with the §18 authority transition fired:
...
Plan 214); four safe dependabot PRs (`#18` tokio 1.53.1, `#21` flate2
1.1.10, `#22` thiserror 2.0.20, `#23` dtolnay/rust-toolchain 1.120.0)
landed on top of the Plan 215 closure SHA `1992d67`; the re-closure
hosted full lane recorded `P213-N-passed` + `P214-N-passed` twice on
identical SHA `a71e0193` (run `35347780246` + run `35349313549`,
73/73 rows green including the `plan214-clean-resource-baseline`
and `plan214-cleanup-kills-on-timeout` rows); the two
`remote-independent-*` rows flip from `blocked` to
`passed-hosted-double-pass-on-a71e0193`; Plan 213 is passed (generic
router-backed Direction A+B, two hosted runs on exact commit
ef59fb3); Plan 201 remains the active M6-Java blocker; Plan 204
stays deferred until Plan 201 records the terminal
`P200-{A..H}` classification and lands its narrow corrective.**
Plan 193 is
closed (`passed-m6-i2pd-mixed-router-streaming`; see
