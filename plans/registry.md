...
| Service tunnels | active | `plans/subsystems/service-tunnels-roadmap.md` | Plan 215 passed; Plan 204 convergence open | Blocked on independent M6 Java second-family closure; Plan 220 passed the diagnostic-attribution corrective, Plan 221 owns the narrowing |
| M6 mixed-router interop | active | `plans/subsystems/mixed-router-interop-roadmap.md` | Plan 220 passed; Plan 221 ready: client-NetDB/OCMOSJ narrowing | Plan 218 reverse-delivery stop narrowed to the helper client-message layer; Plan 219 attribution superseded; Plan 221 is the diagnostic narrowing pass |
...
- **M6 mixed-router program**: Plan 217 closed the harness corrective; Plan 218 stopped at Java→i2pr reverse delivery; Plan 219 diagnostic instrumentation is retained but its J219-B attribution is superseded and refuted on corrected evidence; Plan 220 passed the diagnostic-attribution corrective (`P220-OBSERVABILITY-GAP-CLIENT-NETDB` on exact head `a3d2571`); Plan 221 is the registered client-NetDB/OCMOSJ narrowing pass. No topology/bootstrap corrective is authorized until Plan 221 narrows the gap.

## Dependency-ready and active plans

| Subsystem | Plan | State | Handoff | Dependencies / handoff note |
|---|---|---|---|---|
| M6 mixed-router interop | 221 client-NetDB/OCMOSJ narrowing | ready | `plans/implementation/mixed-router-interop/221-m6-java-client-netdb-ocmosj-narrowing.md` | Plan 220 closed with `P220-OBSERVABILITY-GAP-CLIENT-NETDB` on exact head `a3d2571`; all earlier stages `Known(pass)`. Correlate the reverse send through WP-F-permitted evidence only before any protocol or topology corrective. |

## Blocked work

| Subsystem | Plan | Blocker |
|---|---|---|
| M6 mixed-router interop | 201 | Blocked pending Plan 221 client-NetDB/OCMOSJ narrowing; no J219-B bootstrap corrective was or is authorized (J219-B refuted by Plan 220) |
| Service tunnels | 204 | Cross-milestone convergence waits for M6 Java second-family closure; Plan 220 is diagnostic-only and does not itself unblock convergence |
...
- **Plan 205 SAM/helper pivot** — `retained-deferred-conditional-after-plan218-direct-i2cp-requalification`. The Plan 219 J219-B attribution is superseded; Plan 205 remains deferred until Plan 220 supplies a trustworthy direct-I2CP boundary.

## Recently closed work

| Subsystem | Plan | State | Closure record |
|---|---|---|---|
| M6 mixed-router interop | 220 Plan 219 diagnostic-attribution corrective | closed | `plans/closure/mixed-router-interop/220-status.md` (`passed-m6-java-plan219-diagnostic-attribution-corrective`, commits `b5b049c` + `a3d2571`, terminal `P220-OBSERVABILITY-GAP-CLIENT-NETDB` on exact head `a3d2571`) |
| M6 mixed-router interop | 218 Java second-family final qualification | stopped | `plans/closure/mixed-router-interop/218-status.md` (reverse-delivery boundary; narrowed by Plan 220 to the helper client-message layer) |
...
Full per-plan history (000–220) lives in the subsystem roadmaps §7 tables.
...
- `m6_sequence = 183 -> 184 -> 185 -> 186 -> 187 -> 188 -> 190 -> 191 -> 192 -> 193 -> 196 -> 197 -> 194 -> 198 -> 200 -> 201 -> 217 -> 218 -> 219 -> 220 -> 221` (Plan 205 retained off-path; Plan 220 closed the diagnostic corrective, Plan 221 owns the client-NetDB/OCMOSJ narrowing).
- `next_executable_plan = 221-m6-java-client-netdb-ocmosj-narrowing`; no bootstrap/topology/protocol corrective is authorized until Plan 221 emits narrowed evidence.
