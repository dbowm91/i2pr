...
| Service tunnels | active | `plans/subsystems/service-tunnels-roadmap.md` | Plan 215 passed; Plan 204 convergence open | M10 product authority is closed; Plan 204 remains blocked on independent M6 Java second-family closure pending Plan 245. |
| M6 mixed-router interop | active | `plans/subsystems/mixed-router-interop-roadmap.md` | Plan 244 closed; Plan 245 ready | Plan 245 tests whether Plan 244's retransmit-timer construction proxy was a false negative by observing ConnectionDataReceiver.buildPacket directly, then resumes reverse-direction attribution if construction is proven. |
...
- **M6 mixed-router program**: Plan 217 closed the harness corrective; Plan 218 retains the reverse-delivery behavioral stop; Plan 219 attribution is superseded; Plan 220 refuted J219-B; Plan 221 is superseded-before-execution; Plan 222 narrowed OCMOSJ to status 17; Plan 223 corrected Destination/LS2 separation and moved the tracked send to `ACCEPTED -> NO_LEASESET (21)`; Plan 224 closed with `P224-OBSERVABILITY-GAP-LOOKUP-PATH`; Plan 225 closed with exact `P225-ATTRIBUTION-A-SEARCH-EXHAUSTED-WITHOUT-QUERYING-B`; Plan 226 closed with exact `P226-BASELINE-B-ZERO-HOP-UNKNOWN` and did not admit topology correction; Plan 227 closed with `P227-EXPLICIT-ONE-HOP-NOT-BUILT` (Router C selectable on all 3 counted attempts, no one-hop tunnels built within the five-minute ceiling); Plan 228 closed with `P228-ATTRIBUTION-NO-PAIRED-TUNNEL direction=both` (client configs through C proven, paired tunnel unavailable in both directions on all 4 counted attempts); Plan 229 closed with `P229-C-NOT-EXPLORATORY-ELIGIBLE` (roles + A-only small-router profile proven live, tier population empty on all 4 counted attempts on `00dc368`/`10d1015`); Plan 230 closed with `P230` reverse-delivery boundary (predicate proven, stock C1+C2 correction, natural bootstrap 2/3, forward delivery digest-matched once, reverse admitted-but-unarrived on `09a2f6e`/`3ef5bac`). Plan 231 closed with `P231-C-TARGET-IBGW-NOT-INSTALLED` (post-`ACCEPTED` attribution complete on `1f5a495`: A enqueue proven, C OBEP processing proven, exact target IBGW on B proven absent twice, i2pr wire counters honestly zero; root-caused to the test-driver lease fixture advertising gateway B vs actual inbound gateway A; no production change). Plan 232 closed at Outcome B on `236ccb6` (route-derived lease-gateway correction at all 3 sites; raw-Destination reverse digest-matched `P232-D-REVERSE-DELIVERY-PASSED` on attempt 1 with role flip B -> A; Streaming initial parity proven live with SYN sent on attempts 2+3, both stopping identically at SYN-ACK-never-established). Plan 233 is superseded-before-execution because its family-pass outcome did not reconcile the still-required Plan-200/201 client-LS2 lifecycle rows. Plan 234 closed at `passed-m6-java-streaming-syn-ack-attributed-with-java-accepted-no-response-boundary` after three same-SHA counted attempts emitted `P234-B-JAVA-ACCEPTED-NO-RESPONSE-OBSERVED`. Plan 235 closed at `passed-m6-java-streaming-post-accept-response-boundary-attributed-no-i2pr-inbound` after three same-SHA counted attempts showed Java socket-surface readiness, i2pr outbound admission/dispatch, and zero inbound TunnelData. Plan 236 closed at `P236-C-JAVA-RESPONSE-EMISSION-OBSERVABILITY-GAP` after two same-SHA attempts with source lock passed and response-stage observation incomplete. Plan 237 closed with real stock-Java response observations; Plan 238 then proved Router-A I2CP admission (`client.distributeTime +3`) with dispatch deltas zero on three counted runs. Plan 239 closed at `P239-D-TARGET-LEASESET-LOOKUP-FAILED`. Plan 240 closed at `P240-C-B-ZERO-HOP-UNKNOWN-RI` after reusing the P225/P226 exact target-job trace in the live Streaming epoch and classifying the first Router-B boundary (B eligible, in `toTry`, zero-hop-unknown skip, never queried) on `37025f9` x3. Plan 241 is closed at the A/B build-stage boundary. Plan 242 then closed the selector-semantics/bootstrap/pair-gate corrective, but live external attempts were unexecuted on its closure host. Plan 243 is closed with Direction A established. Plan 244 is closed with the reverse direction bounded repeatably at `P244-B-RESPONSE-PACKET-NOT-CONSTRUCTED` on three counted same-SHA hosted executions with fully bound response epochs. Exact-pinned source review shows the retained `Resend in` construction proxy is conditional, so Plan 245 is registered-ready to observe `ConnectionDataReceiver.buildPacket` directly and distinguish an observer false negative from real response suppression. M6 Java remains not-yet-passed.

## Dependency-ready and active plans

Current authority: Plan 244 closed the continuous response attribution on `154e92d8436dbad0b020a0b19d848d2d227a22af`, but exact-pinned source review shows its `Resend in`-based construction signal is conditional and absent for ACK-only sequence-0 non-SYN packets. Plan 245 is dependency-ready to add a direct `ConnectionDataReceiver.buildPacket` construction observation, classify writeData suppression if present, and resume the same-epoch reverse chain when direct construction is proven. Plan 201 and Plan 204 remain blocked on Java-family closure.

| Subsystem | Plan | State | Handoff | Dependencies / handoff note |
|---|---|---|---|---|
| M6 mixed-router interop | 245 stock-response construction-signal attribution corrective | ready | `plans/implementation/mixed-router-interop/245-m6-java-streaming-stock-response-construction-signal-attribution-corrective.md` | Directly observe ConnectionDataReceiver.buildPacket; do not patch Java or production i2pr; resume Plan-244 reverse chain if construction is proven. |

## Blocked work

| Subsystem | Plan | Blocker |
|---|---|---|
| M6 mixed-router interop | 201 | Blocked on M6 Java Streaming reverse direction pending Plan 245 plus publication / final-closure axis |
| Service tunnels | 204 | Cross-milestone convergence waits for M6 Java second-family closure pending Plan 245; closed M10 product authority unchanged |
...
Full per-plan history (000–245) lives in the subsystem roadmaps §7 tables.
