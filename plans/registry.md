...
| Service tunnels | active | `plans/subsystems/service-tunnels-roadmap.md` | Plan 215 passed; Plan 204 convergence open | M10 product authority is closed; Plan 204 remains blocked on independent M6 Java second-family closure pending Plan 242. |
| M6 mixed-router interop | active | `plans/subsystems/mixed-router-interop-roadmap.md` | Plan 241 closed; Plan 242 ready | Exact-pinned Java proves explicitPeers is sampled only 1-in-4 builds. Plan 242 removes exact-via-C as a false hard gate, accepts a genuine non-zero-hop client pair, and resumes the retained lookup chain. |
...
Current authority: Plan 241 passed the one-hop client-tunnel fixture corrective at the A/B build-stage boundary. Exact-pinned Java source now proves `explicitPeers` is selected only when `random.nextInt(4) == 0`; Plan 241 therefore over-constrained continuation by requiring every tunnel to be exact-via-C. Plan 242 is dependency-ready to accept any controlled non-zero-hop client pair, keep C-use diagnostic, and resume the exact Plan-240/241 lookup continuation. Plans 201 and 204 remain blocked on Java-family closure.

| Subsystem | Plan | State | Handoff | Dependencies / handoff note |
|---|---|---|---|---|
| M6 mixed-router interop | 242 Streaming stock one-hop selector semantics corrective | ready | `plans/implementation/mixed-router-interop/242-m6-java-streaming-stock-one-hop-selector-semantics-corrective.md` | Source-lock the 1-in-4 explicit-peer semantics, remove exact-via-C as a hard gate, observe actual controlled tunnel paths, then resume the retained lookup/OCMOSJ chain. |

## Blocked work

| Subsystem | Plan | Blocker |
|---|---|---|
| M6 mixed-router interop | 201 | Blocked after Plan 241 A/B build-stage boundary; streaming axis pending Plan 242, publication/final-closure axis unchanged |
| Service tunnels | 204 | Cross-milestone convergence waits for M6 Java second-family closure pending Plan 242; closed M10 product authority unchanged |
