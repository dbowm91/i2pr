...
| Service tunnels | active | `plans/subsystems/service-tunnels-roadmap.md` | Plan 215 passed; Plan 204 convergence open | M10 product authority is closed; Plan 204 remains blocked on independent M6 Java second-family closure pending the Plan-240-authorized zero-hop successor. |
| M6 mixed-router interop | active | `plans/subsystems/mixed-router-interop-roadmap.md` | Plan 240 passed at B-ZERO-HOP-UNKNOWN-RI; zero-hop successor pending | Plan 240 proved the first B-specific pre-query guard (B eligible, in `toTry`, zero-hop-unknown skip, never queried) on `37025f9` x3. A narrow zero-hop-tunnel/RI-availability successor needs its own plan-of-record. |
...
Full per-plan history (000–240) lives in the subsystem roadmaps §7 tables.
