...
| M6 mixed-router interop | active | `plans/subsystems/mixed-router-interop-roadmap.md` | Plan 240 passed at B-ZERO-HOP-UNKNOWN-RI; Plan 241 ready | Plan 241 corrects only the Streaming helper's forced zero-hop client profile, requires the Plan-230 one-hop-via-C bootstrap gate, and resumes the exact Plan-240 lookup chain. |
...
Current authority: Plan 240 passed the streaming-epoch lookup-failure attribution at `P240-C-B-ZERO-HOP-UNKNOWN-RI` (exact streaming ISJ correlated, B eligible and in `toTry`, zero-hop-unknown skip proven, never queried, on `37025f9` x3). Plan 241 is dependency-ready to replace only the forced zero-hop Streaming helper profile with the already-proven one-hop-via-C SessionConfig and then resume the exact lookup chain. Plans 201 and 204 remain blocked on Java-family closure.

| Subsystem | Plan | State | Handoff | Dependencies / handoff note |
|---|---|---|---|---|
| M6 mixed-router interop | 241 Java Streaming one-hop client-tunnel fixture corrective | ready | `plans/implementation/mixed-router-interop/241-m6-java-streaming-one-hop-client-tunnel-fixture-corrective.md` | Requires Plan-230 transit/bootstrap readiness, genuine one-hop client pair through C with zero-hop absent, main-vs-client B-RI visibility, then exact Plan-240 lookup continuation. |

## Blocked work

| Subsystem | Plan | Blocker |
|---|---|---|
| M6 mixed-router interop | 201 | Blocked after Plan 240 B-Zero-Hop-Unknown-RI boundary; streaming axis pending Plan 241, publication/final-closure axis unchanged |
| Service tunnels | 204 | Cross-milestone convergence waits for M6 Java second-family closure pending Plan 241; closed M10 product authority unchanged |
