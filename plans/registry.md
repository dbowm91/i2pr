...
| Service tunnels | active | `plans/subsystems/service-tunnels-roadmap.md` | Plan 215 passed; Plan 204 convergence open | M10 product authority is closed; Plan 204 remains blocked on independent M6 Java second-family closure pending Plan 241. |
| M6 mixed-router interop | active | `plans/subsystems/mixed-router-interop-roadmap.md` | Plan 241 passed at the A/B build-stage boundary; §20 successor open | Plan 241 corrected only the Streaming helper's forced zero-hop client profile, required the Plan-230 one-hop-via-C bootstrap gate, and resumed the exact Plan-240 lookup chain up to the installed-pair gate. |
...
Full per-plan history (000–241) lives in the subsystem roadmaps §7 tables.
