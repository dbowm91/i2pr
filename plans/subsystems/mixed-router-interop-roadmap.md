...
Historic/registered plans: 183–194, 196–198, 200, 201, 205, 217–236 (global i2pr numbers, preserved).
...
Plan 234 is closed at Outcome B with `P234-B-JAVA-ACCEPTED-NO-RESPONSE-OBSERVED`. Plan 235 is also closed at Outcome B with `P235-B-JAVA-SOCKET-SURFACE-READY-NO-I2PR-INBOUND`: Java's public socket surface was ready and i2pr admitted the outbound SYN, but no inbound TunnelData appeared. Plan 236 is registered-ready to source-lock and observe the exact stock-Java response path from scheduler/packet construction through `Connection.sendPacket` → `PacketQueue.enqueue` → `I2PSession.sendMessage`, then continue into Router-A client-message/tunnel attribution only after Java send is proven. The retained Plan-232 raw-Destination reverse pass remains authoritative; Java-family M6 remains not-yet-passed.
...
```text
183 -> 184 -> 185 -> 186 -> 187 -> 188 -> 190 -> 191 -> 192 -> 193 -> 196 -> 197 -> 194 -> 198 -> 200 -> 201 -> 217 -> 218 -> 219 -> 220 -> 221 -> 222 -> 223 -> 224 -> 225 -> 226 -> 227 -> 228 -> 229 -> 230 -> 231 -> 232 -> 233 -> 234 -> 235 -> 236. Plan 205 is retained conditional fallback and is not part of the primary sequence.
```
...
| 201 | blocked | blocked-pending-plan236-java-streaming-response-emission-attribution | — | `plans/closure/mixed-router-interop/201-m6-java-public-client-publication-corrective-and-second-family-closure.md`; `plans/closure/mixed-router-interop/201-status.md` |
...
| 236 | ready | registered-ready-m6-java-streaming-response-emission-and-i2cp-send-attribution | `plans/implementation/mixed-router-interop/236-m6-java-streaming-response-emission-and-i2cp-send-attribution.md` | `plans/closure/mixed-router-interop/236-status.md` |
