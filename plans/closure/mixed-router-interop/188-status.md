...
Status: **`blocked-by-plan191-and-plan192`**
...
from `blocked` to `passed`; Plan 191 ran the inbound-delivery
layer and stopped at the i2pd-compatible I2CP-style Data body
wire-format defect (2 inbound-delivery rows recorded `blocked`
with Plan 191 §6 stop provenance; the 2 ordering rows
`external-direct-rejected` and `external-liveness-first-test`
now `passed`); Plan 192 is the narrower registered follow-up
that owns the wire-format fix; no Streaming claim). See
[`plans/closure/mixed-router-interop/190-status.md`](190-status.md) for the closed reply-path
corrective, [`plans/closure/mixed-router-interop/191-status.md`](191-status.md) for the inbound
boundary E documentation, and
[`plans/implementation/mixed-router-interop/192-m6-i2cp-wire-format-corrective.md`](../../implementation/mixed-router-interop/192-m6-i2cp-wire-format-corrective.md)
for the registered follow-up.

Plan of record:
[`plans/implementation/mixed-router-interop/188-m6-short-build-reply-interop-corrective.md`](../../implementation/mixed-router-interop/188-m6-short-build-reply-interop-corrective.md).

> Numbering note (as of Plan 190): `plans/implementation/mixed-router-interop/188-m6-mixed-router-streaming-with-i2pd.md`
...
> closes against.
> `plans/implementation/mixed-router-interop/189-m6-java-i2p-second-family-qualification-and-closure.md`
...
plan_188 = blocked-by-plan191-and-plan192 (real outbound/inbound i2pd installs retained-passed; reply-path correction retained-passed via plan190; inbound-delivery layer now blocked on plan192)
m6_destination_remote_interop = installs-proven-lookup-publication-outbound-passed-inbound-delivery-blocked-plan191-then-plan192
