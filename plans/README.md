...
m6_inbound_netdb_reply_path_correction = passed-via-plan190 (typed InboundGatewayRoute + daemon-owned adapter; remote lane flips 3 destination rows blocked -> passed)
m6_inbound_destination_delivery = passed-via-plan192 (i2cp-compatible I2CP-style Data wire-format; STYLE=RAW SAM session; 9-byte short-transport inner envelope)
m6_mixed_router_cross_family_ledger = closed-via-plan194-second-family-qualification (i2pd-family-passed-via-plan193; java-second-family-passed-via-plan194-with-bounded-sam-ls2-gap; pq-parser-tolerance-landed-via-plan197)
 m6_ssu2_pq_option_tolerance = landed-via-plan197-typed-parser-surface (Ssu2RouterAddress::parse accepts Java `pq=4,3`; typed PqCapabilities surfaced on every parsed address via `pq_capabilities()` accessor; first-family i2pd 2.61.0 lane stays green because i2pd does not publish pq)
 m6_java_second_family_qualification = topology-and-authenticated-ssu2-preflight-and-sam-bridge-and-tunnel-install-passed-via-plan194-with-bounded-sam-ls2-publication-gap
...
- [`194-m6-java-second-family-mixed-router-closure.md`](194-m6-java-second-family-mixed-router-closure.md) — **passed-m6-java-second-family-mixed-router-closure-with-sam-ls2-gap** the M6 Java I2P second-family qualification and mixed-router closure plan (see [`194-status.md`](194-status.md)): lands the §3 fetch script (`scripts/interop/fetch-m6-java.sh`; exact-pinned Java I2P 2.13.0 @ `9134f808337b401e8e53c73734c81fab04280c9d` built via `ant updater preppkg`, no IzPack 5 GUI installer), the §5 second-family harness (`tests/integration/m6-interop/run-java.sh`; fresh disposable datadir, no reseed, SAM loopback; runs both `destination_message_plane_against_java` and `streaming_through_java` drivers and aggregates evidence keys), the §5.1-§5.5 external drivers (`crates/i2pr-daemon/tests/java_tunnel_external.rs::destination_message_plane_against_java` + `streaming_through_java`), the cross-family aggregator wiring (`tests/integration/m6-interop/run-m6-mixed-router.sh` binds every Plan 189 §8 guarded row to a family + the actual `run-java.sh` exit code), the static structural checker extension (`scripts/check-m6-mixed-router-acceptance-evidence.sh` + `scripts/check-streaming-tunnel-evidence.sh` + `scripts/check-destination-tunnel-evidence.sh` each require both i2pd + Java harnesses), and the hosted-workflow extension (`.github/workflows/m6-mixed-router-external.yml` runs the Java fetch + build log tail). Fresh external `run-java.sh` against the exact-pinned Java I2P 2.13.0 cache proves 26 rows passed (local + every Plan 196 topology row + authenticated-session + SAM-destination-creation + tunnel-build-install + manager-cleanup) + 22 rows blocked with `plan194-java-stop` provenance (LS2-lookup-dependent + delivery-dependent + streaming-layer) + 0 failed rows. The bounded M6 claim is `milestone6_java_mixed_router_interop = passed-via-plan194-with-sam-ls2-publication-gap`: Java's SAM bridge does NOT auto-publish the SAM-destination LS2 to the local NetDB in a controlled private topology where the reference has no peer tunnels to build a client tunnel for the lease; i2pd's SAM bridge does publish immediately; the gap is a Java-internal architectural fact, not an i2pr regression. Plan 195 (M10 remote service interop) is unblocked.
