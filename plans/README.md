...
plan_211 = retained-source-harness-blocked-by-plan212 (source harness retained; closure blocked until Plan 212 generic A+B passes and this lane requalifies — see plans/212-status.md)
plan_212 = in-progress-source-closure-landed-external-qualification-pending (router-backed state + per-service provisioning + canonical inbound Streaming source landed; 25 plan212 unit rows + §20–§26 checker + ignored generic A/B driver; external i2pd lane pending)
plan_213 = in-progress-commit-e-local-double-pass-hosted-double-run-pending (generic A/B application driver with real TCP I/O + per-direction counter windows, harness-only SAM STREAM fixture, standalone fail-closed runner, §27 checker, router-generic workflow lane; lane-proven narrow correctives + two consecutive local `P213-N-passed` runs; hosted double-run pending — see plans/213-status.md)
m10_remote_transport_core = not-yet-passed (Plan 212 generic router-backed Direction A+B external proof pending; Plan 202 closed the typed routing classification + bounded counters; Plan 206 attached the executable backend; Plan 208 wired the backend into the production `deliver_outbound` sweep so the manager routes its own queued Streaming requests through the real Plan 184–193 router stack — a reachable remote peer no longer dies at the pre-Plan-208 `unknown_peer` terminal branch)
m10_remote_application_interop = not-yet-passed (blocked until Plan 212 passes and Plan 211 requalifies; Plan 211 ships the source-side closure with real enabled `HttpClient` + `IrcClient` service specs whose destinations are the i2pd server destinations extracted from the per-tunnel `.dat` files via the bounded `parse_i2pd_destination.py` helper; the driver emits the documented Plan 211 §10 subfact rows through before/after production remote-counter snapshots; the static checker `scripts/check-service-tunnel-acceptance-evidence.sh` is extended with the Plan 211 §13 anti-shadow rules; the dedicated M6 interop lane that provisions i2pd with peers is the authoritative external qualification surface that flips the two `remote-independent-*` rows from `blocked` to `passed`)
...
m6_java_second_family_qualification = public-client-corrective-diagnosed-via-plan200-and-blocked-on-plan201 (Plan 198 helpers connect; Plan 200 closed the diagnostic/evidence side with one terminal `P200-{A..H}` classification per run; Plan 201 landed the Branch G corrective framework; downstream rows stay blocked until the Plan 200 exact-head external run consumes the `P200-*` classification and the matching Plan 201 branch lands)
 ...
 milestone6_interoperable = not-yet-claimed
  next_product_layer = milestone11-planning-pending-m6-java-second-family-closure (Plan 210 closed the real per-service destination tunnel material + inbound Streaming corrective; Plan 211 ships the source-side closure with real enabled HttpClient + IrcClient service specs + i2pd server-destination `.dat`-file extraction + documented §10 subfact emission through before/after production remote-counter snapshots + static-checker §13 anti-shadow rules; the dedicated M6 interop lane that provisions i2pd with peers is the authoritative external qualification surface that flips the two `remote-independent-*` rows from `blocked` to `passed`; the remaining M10 work is the Plan 201 Java corrective plus the Plan 204 final closure transition; both are blocked on Plan 201's exact-head external run)
