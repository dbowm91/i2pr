...
m10_remote_application_interop = passed-via-plan209 (Plan 209 deleted the shadow router stack the Plan 207 driver constructed and replaced it with the production composition helper `ServiceProduct::start`; the counted application driver is a black-box product harness that never touches the lower-stack types listed in Plan 209 §5; aggregate rows derive purely from the documented Plan 209 §11 subfact rows)
