...
plan_214 = local-pass-proven-hosted-double-pass-pending (black-box v214 driver with pumped subprocesses + target-derived HTTP/IRC facts + 21 plan214 unit rows, standalone `run-plan214-applications.sh` runner with §16/§17 aggregates + exactly one `P214-*` classification, §28 checker incl. synthetic-key extraction self-test, workflow Plan 213 → Plan 214 ordering; Commit E added the typed remote-mirror resolution seam + 600 s fixture windows + transparent i2pd server tunnel with tap attribution + neutral-shape privacy proof + evidence truncation; first local `P214-N-passed`; hosted double-pass on one exact SHA pending — see plans/214-status.md)
...
m10_remote_application_interop = locally-passed-once (Plan 214 P214-N on the Commit E tree: black-box v214 driver + pumped curl/jaraco subprocesses + fresh-seq target facts + independent counter windows + standalone runner + §28 checker; hosted double-pass on one exact SHA pending)
