...
The current **next executable plan** is Milestone 9 planning.
[**Plan 161**](161-m8-ssu2-independent-ipv4-interop-and-final-closure.md)
has **passed** the final independent direct-session gate and closed
Milestone 8 within its bounded scope. Plan 162 passed its narrow
corrective: the environment-dependent Plan 161
...
[**Plan 154**](154-status.md) (closed with Plan 161). Plans 155–161 have
passed: directions A
and B (plus cached-token/malformed rows) are proven against exact-pinned
i2pd 2.61.0, and the fail-closed 15-row ledger, its routine-CI-enforced
checker, and the manual external workflow pass locally and hosted.
...
- [`155-m8-ssu2-v2-protocol-foundation-and-addresses.md`](155-m8-ssu2-v2-protocol-foundation-and-addresses.md) — **passed** runtime-neutral SSU2 v2 address/header/block foundation and generic `TransportKind::Ssu2` integration.
- [`156-m8-ssu2-v2-handshake-token-and-routerinfo.md`](156-m8-ssu2-v2-handshake-token-and-routerinfo.md) — **passed** Noise XK establishment, header protection, TokenRequest/Retry, bounded one-use token lifecycle, RouterInfo fragmentation/validation, replay/deadline state.
- [`157-m8-ssu2-v2-data-phase-reliability-and-fragmentation.md`](157-m8-ssu2-v2-data-phase-reliability-and-fragmentation.md) — **passed** authenticated data packets, ACK/loss/congestion/retransmission, I2NP fragmentation/reassembly, duplicate suppression, termination/rekey.
- [`158-m8-ssu2-udp-runtime-and-local-session-product.md`](158-m8-ssu2-udp-runtime-and-local-session-product.md) — **passed** production UDP ownership in `i2pr-runtime`, existing `TransportManager` integration, central bounded scheduler, and real localhost i2pr↔i2pr UDP product tests.
...
- [`160-m8-ssu2-peer-test-and-relay-reachability.md`](160-m8-ssu2-peer-test-and-relay-reachability.md) — **passed** PeerTest and relay requester/introducer/target roles, anti-amplification/resource policy, introducer records, and real-loopback NAT-like acceptance without namespaces.
 - [`161-m8-ssu2-independent-ipv4-interop-and-final-closure.md`](161-m8-ssu2-independent-ipv4-interop-and-final-closure.md) — **passed** final independent direct-session gate and Milestone 8 closure. Directions A and B are proven against exact-pinned i2pd 2.61.0 (`635b013a612ff47278ef02acf8580a28e10e26c5`) over real localhost UDP with authenticated small + fragmented I2NP exchange and DeliveryStatus return traffic per direction, plus cached-token and malformed/resource rows. The fail-closed 15-row evidence ledger (`tests/integration/ssu2/run-independent.sh`), its integrity checker (`scripts/check-ssu2-acceptance-evidence.sh`, routine-CI-enforced), and the manual `.github/workflows/ssu2-external.yml` lane pass locally and hosted (routine CI runs `34050058216`/`34053041778`, external runs `34051298144`/`34053042857`); Java I2P is recorded nonblocking debt. Scope stays bounded: no public-network, NetDB/tunnel/destination, advertisement, IPv6-external, PQ, SSU1, or Milestone 6 interop claim.
...
- SSU2 v2 local protocol/runtime/reachability product through Plan 160.
 - Plan 161 closed Milestone 8: authenticated direct IPv4 SSU2 v2 interop against exact-pinned i2pd 2.61.0 with small + fragmented I2NP exchange and return DeliveryStatus traffic per direction, plus cached-token and malformed/resource rows, plus the passing fail-closed evidence ledger/checker/manual-workflow lane (Java recorded nonblocking debt).

## What's not yet accepted

 - Live/public NTCP2 or SSU2 router transport activation and broad mixed-router interoperability.
- Anything outside Plan 161's bounded direct-interop scope: public I2P participation, NetDB/tunnel/destination interop, IPv6-external, PQ, SSU1, Milestone 6 interop.
...
```text
Plan 161 passed; Milestone 8 closed within its bounded direct-interop scope
Plan 162 passed its narrow external-test lane/CI corrective
Plan 154 closed as the Milestone 8 roadmap authority
Plans 155–161 passed; next product layer is milestone9-planning
```
