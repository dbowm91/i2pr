...
The current implementation's RFC-4648 `+/` SAM encoding is a protocol defect. Current official SAM documentation requires the I2P Base64 alphabet using `-` and `~`. Plan 142 owns correction and independent compatibility evidence.

### Private destination

The current 455-byte type-7/type-4 private-destination representation is not automatically rejected, but its existing evidence is circular. Plan 142 must reconcile it against an independently generated/reference artifact before restoring an interoperability claim.
...
The next implementation model should read Plan 141 and execute **Plan 142 only**.
