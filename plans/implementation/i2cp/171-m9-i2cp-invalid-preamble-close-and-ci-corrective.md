...
Confirm whether the host-sensitive behavior is caused by relying on `TcpStream` drop after `handle_connection_inner()` returns, by task/scope teardown ordering, or by the paused-time test harness. Record the finding in `plans/closure/i2cp/171-status.md`.
...
17. `plans/closure/i2cp/171-status.md` records exact commands, failure diagnosis, closing SHA, and hosted run ID.
...
```text
plans/closure/i2cp/171-status.md
plans/README.md
README.md
AGENTS.md
