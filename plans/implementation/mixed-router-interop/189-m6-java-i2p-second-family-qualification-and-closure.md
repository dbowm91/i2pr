...
12. `plans/closure/mixed-router-interop/189-status.md` may set `milestone6_interoperable = passed-via-plan189` and advance the handoff. The next executable plan after Plan 189 will be the (currently unregistered) plan that resumes Plan 181's two remote M10 application rows.
