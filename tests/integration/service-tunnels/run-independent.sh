#!/usr/bin/env bash
# Plan 199 / Plan 181 / Plan 207 — M10 independent-application-client
# matrix plus the controlled remote independent-I2P qualification
# attempt and the Plan 207 genuine remote HTTP + IRC application
# interoperability corrective driver.
#
...
#   (Plan 207: real system curl + exact-pinned jaraco/irc subprocess
#   invocations against the production M10 listeners; aggregate
#   pass rows derived from command exit codes + parsed application
#   results + remote fixture facts + Plan 206 backend counters —
#   never from synthetic label-injection).
#
...
threshold = 0
EOF
    # Plan 207 §5 — i2pd provisions one HTTP server tunnel and one
    # IRC server tunnel that point at the harness-owned loopback
    # fixtures. The HTTP destination and IRC destination keys are
    # generated by the i2pd process on first start; the Plan 207
    # driver consumes those destinations through i2pd's public SAM
    # bridge (DEST GENERATE + SESSION CREATE).
    cat > "${I2PD_HOME}/tunnels.conf" <<EOF
[HTTP-Server]
type = http
host = 127.0.0.1
port = ${HTTP_TARGET}
keys = plan207-http-server.dat
inbound.length = 1
outbound.length = 1
...
PY
      ...
      if [[ "${sam_rc}" -ne 0 || ! -s "${I2PD_PUB_FILE}" ]]; then
        ...
        # Plan 207 — genuine remote HTTP + IRC application
        # interoperability corrective. The driver replaces the
        # synthetic Plan 203 label-injection with command-derived
        # evidence from unmodified application clients. The HTTP
        # row is bound to the real system `curl` binary; the IRC row
        # is bound to the unmodified jaraco/irc Python public API
        # (`irc.client`) installed from the exact-pinned
        # jaraco/irc checkout. The aggregate pass row is derived
        # purely from the documented Plan 207 §9 subfact rows the
        # driver writes to `${EVIDENCE_DIR}/plan207-driver/driver-
        # evidence.tsv`; a missing subfact or a synthetic `record
        # "... passed"` literal fails this lane closed.
        PLAN207_LOG="${EVIDENCE_DIR}/plan207-remote-application.log"
        : > "${PLAN207_LOG}"
        plan207_rc=0
        if I2PD_ROUTER_INFO="${I2PD_RI}" \
           I2PD_SSU2_ENDPOINT="127.0.0.1:${I2PD_PORT}" \
           I2PR_SSU2_BIND="127.0.0.1:${I2PR_SSU2_BIND_PORT}" \
           I2PD_SAM_ENDPOINT="127.0.0.1:${SAM_PORT}" \
           EVIDENCE_DIR="${EVIDENCE_DIR}/plan207-driver" \
           PLAN207_HTTP_TARGET_PORT="${HTTP_TARGET}" \
           PLAN207_IRC_TARGET_PORT="${IRC_TARGET}" \
           PLAN207_JARACO_SRC="${JARACO_SRC}" \
           PLAN207_HARNESS_DIR="${LANE_DIR}" \
           timeout --foreground 180s \
           cargo test --locked -p i2pr-daemon --test service_tunnels_application_genuine_remote_qualification \
           m10_genuine_remote_http_and_irc_application_interop -- --ignored --exact --nocapture --test-threads=1 \
           >>"${PLAN207_LOG}" 2>&1; then
          plan207_rc=0
        else
          plan207_rc=$?
        fi
        # Plan 207 §10 fail-closed shape: missing required env (or a
        # missing SAM/listener) blocks both remote rows. The presence
        # of "missing required env" in the driver log proves the
        # lane was attempted with the right env shape; the runner
        # must not retry.
        if grep -Fq "missing required env" "${PLAN207_LOG}" ||
           grep -Fq "lane requires a fixed loopback bind" "${PLAN207_LOG}" ||
           grep -Fq "i2pd SAM did not listen" "${PLAN207_LOG}"; then
          record_blocked "remote-independent-http-eepsite" \
            "Plan 207 positive remote HTTP eepsite driver fail-closed: required SSU2 lane env (I2PD_ROUTER_INFO / I2PD_SSU2_ENDPOINT / I2PR_SSU2_BIND / I2PD_SAM_ENDPOINT / EVIDENCE_DIR / PLAN207_HTTP_TARGET_PORT / PLAN207_IRC_TARGET_PORT / PLAN207_JARACO_SRC / PLAN207_HARNESS_DIR) not satisfied in this lane (see plan207-remote-application.log)"
          record_blocked "remote-independent-irc-service" \
            "Plan 207 positive remote IRC service driver fail-closed: required SSU2 lane env not satisfied in this lane (see plan207-remote-application.log)"
        else
          PLAN207_TSV="${EVIDENCE_DIR}/plan207-driver/driver-evidence.tsv"
          PLAN207_EVIDENCE_FILES=("${PLAN207_LOG}" "${PLAN207_TSV}")
          plan207_has_subfact() {
            local key="$1"
            local expected="$2"
            local file value
            for file in "${PLAN207_EVIDENCE_FILES[@]}"; do
              if [[ ! -f "${file}" ]]; then
                continue
              fi
          ...
          # Plan 207 §9 — required HTTP subfacts.
          http_pass=true
          for label in http-command-exit http-status http-fixture-observed \
                       http-no-clearnet-fallback http-policy-retained \
                       http-remote-stream-established http-local-coowned-not-used \
                       http-clean-resource-baseline; do
            if ! plan207_has_subfact "${label}" "1" 2>/dev/null &&
               ! plan207_has_subfact "${label}" "0" 2>/dev/null; then
              http_pass=false
              break
            fi
          done
          # http-command-exit and http-status must equal exactly "0" and "200".
          if ! plan207_has_subfact "http-command-exit" "0"; then
            http_pass=false
          fi
          if ! plan207_has_subfact "http-status" "200"; then
            http_pass=false
          fi
          # http-fixture-observed, http-no-clearnet-fallback, http-policy-retained,
          # http-remote-stream-established, http-local-coowned-not-used, http-clean-resource-baseline
          # must equal "1".
          for label in http-fixture-observed http-no-clearnet-fallback http-policy-retained \
                       http-remote-stream-established http-local-coowned-not-used \
                       http-clean-resource-baseline; do
            if ! plan207_has_subfact "${label}" "1"; then
              http_pass=false
              break
            fi
          done
          # Required HTTP digest rows must be present and 64-char hex.
          for label in http-body-digest http-request-digest http-multipacket-digest; do
            file=""
            for f in "${PLAN207_EVIDENCE_FILES[@]}"; do
              if [[ -f "${f}" ]]; then
          ...
          # Plan 207 §9 — required IRC subfacts.
          irc_pass=true
          for label in irc-driver-exit irc-registration-welcome irc-ping-pong-roundtrip \
                       irc-privmsg-outbound-observed irc-privmsg-inbound-observed \
                       irc-ctcp-action-allowed irc-dcc-blocked \
                       irc-privacy-hostname-rewrite irc-remote-stream-established \
                       irc-local-coowned-not-used irc-clean-resource-baseline; do
            value=""
            for f in "${PLAN207_EVIDENCE_FILES[@]}"; do
              if [[ -f "${f}" ]]; then
                value=$(awk -F'\t' -v want="${label}" '$1 == want {print $2; exit}' "${f}" 2>/dev/null || true)
          ...
          # Plan 206 backend proof.
          plan206_proof=false
          for f in "${PLAN207_EVIDENCE_FILES[@]}"; do
            if [[ -f "${f}" ]] && grep -Fq "plan206_ok=1" "${f}"; then
              plan206_proof=true
              break
            fi
          done
          if [[ "${http_pass}" != true || "${irc_pass}" != true || "${plan206_proof}" != true ]]; then
            record "remote-independent-http-eepsite" failed \
              "Plan 207 positive remote HTTP eepsite guard returned rc=${plan207_rc} without the documented Plan 207 §9 subfacts (http_pass=${http_pass} irc_pass=${irc_pass} plan206_proof=${plan206_proof}; see plan207-remote-application.log + driver-evidence.tsv)"
            record "remote-independent-irc-service" failed \
              "Plan 207 positive remote IRC service guard returned rc=${plan207_rc} without the documented Plan 207 §9 subfacts (http_pass=${http_pass} irc_pass=${irc_pass} plan206_proof=${plan206_proof}; see plan207-remote-application.log + driver-evidence.tsv)"
          else
            record_guarded "remote-independent-http-eepsite" \
              "Plan 207 positive remote HTTP eepsite: real system curl exit=0 + status=200 + body digest matches the loopback HTTP fixture, with Plan 206 backend counters observed (see plan207-remote-application.log + driver-evidence.tsv)" \
              0
            record_guarded "remote-independent-irc-service" \
              "Plan 207 positive remote IRC service: real exact-pinned jaraco/irc public API exit=0 + welcome + ping/pong + PRIVMSG round-trip + CTCP/DCC policy + privacy rewrite, with Plan 206 backend counters observed (see plan207-remote-application.log + driver-evidence.tsv)" \
              0
          fi
        fi
      fi
  ...
  record_blocked "remote-independent-http-eepsite" \
    "not attempted in the local-only lane; run the full lane for the §6 + Plan 207 qualification"
  record_blocked "remote-independent-irc-service" \
    "not attempted in the local-only lane; run the full lane for the §6 + Plan 207 qualification"
  record_blocked "m10-remote-destination-streaming-composition" \
    "not attempted in the local-only lane; run the full lane for the §6 + Plan 202 + Plan 207 qualification"
fi
