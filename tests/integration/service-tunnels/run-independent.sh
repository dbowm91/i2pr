...
threshold = 0
EOF
    # Plan 209 §6 — i2pd provisions one HTTP server tunnel and one
    # IRC server tunnel that point at the harness-owned loopback
    # fixtures. The HTTP destination and IRC destination keys are
    # generated by the i2pd process on first start; the Plan 209
    # driver consumes those destinations through i2pd's public SAM
    # bridge (DEST GENERATE + SESSION CREATE), declared inside the
    # i2pd-owned server-tunnel configuration. The counted driver
    # never sees the destination keys; the production composition
    # reads them through i2pd's SAM surface only.
...
PY
      ...
      if [[ "${sam_rc}" -ne 0 || ! -s "${I2PD_PUB_FILE}" ]]; then
        ...
        # derived purely from the documented Plan 209 §11 subfact
        # rows the driver writes to
        # `${EVIDENCE_DIR}/plan209-driver/driver-evidence.tsv`;
        # a missing subfact or a synthetic `record "... passed"`
        # literal fails this lane closed.
        PLAN209_LOG="${EVIDENCE_DIR}/plan209-remote-application.log"
        : > "${PLAN209_LOG}"
        plan209_rc=0
        if I2PD_ROUTER_INFO="${I2PD_RI}" \
           I2PD_SSU2_ENDPOINT="127.0.0.1:${I2PD_PORT}" \
           I2PR_SSU2_BIND="127.0.0.1:${I2PR_SSU2_BIND_PORT}" \
           I2PD_SAM_ENDPOINT="127.0.0.1:${SAM_PORT}" \
           EVIDENCE_DIR="${EVIDENCE_DIR}/plan209-driver" \
           PLAN209_HTTP_TARGET_PORT="${HTTP_TARGET}" \
           PLAN209_IRC_TARGET_PORT="${IRC_TARGET}" \
           PLAN209_JARACO_SRC="${JARACO_SRC}" \
           PLAN209_HARNESS_DIR="${LANE_DIR}" \
           ...
           m10_product_only_remote_http_and_irc_application_interop -- --ignored --exact --nocapture --test-threads=1 \
           >>"${PLAN209_LOG}" 2>&1; then
          plan209_rc=0
        else
          plan209_rc=$?
        fi
        # Plan 209 §13 fail-closed shape: missing required env (or
        ...
        if grep -Fq "missing required env" "${PLAN209_LOG}" ||
           grep -Fq "lane requires a fixed loopback bind" "${PLAN209_LOG}" ||
           grep -Fq "i2pd SAM did not listen" "${PLAN209_LOG}"; then
          record_blocked "remote-independent-http-eepsite" \
            "Plan 209 product-only remote HTTP eepsite driver fail-closed: required SSU2 lane env (I2PD_ROUTER_INFO / I2PD_SSU2_ENDPOINT / I2PR_SSU2_BIND / I2PD_SAM_ENDPOINT / EVIDENCE_DIR / PLAN209_HTTP_TARGET_PORT / PLAN209_IRC_TARGET_PORT / PLAN209_JARACO_SRC / PLAN209_HARNESS_DIR) not satisfied in this lane (see plan209-remote-application.log)"
          record_blocked "remote-independent-irc-service" \
            "Plan 209 product-only remote IRC service driver fail-closed: required SSU2 lane env not satisfied in this lane (see plan209-remote-application.log)"
        else
          PLAN209_TSV="${EVIDENCE_DIR}/plan209-driver/driver-evidence.tsv"
          PLAN209_EVIDENCE_FILES=("${PLAN209_LOG}" "${PLAN209_TSV}")
          plan209_has_subfact() {
            local key="$1"
            local expected="$2"
            local file value
            for file in "${PLAN209_EVIDENCE_FILES[@]}"; do
              if [[ ! -f "${file}" ]]; then
                continue
              fi
          ...
          # Plan 209 §11 — required HTTP subfacts.
          http_pass=true
          for label in http-command-exit http-status http-fixture-observed \
                       ...
                       http-local-coowned-not-used http-clean-resource-baseline; do
            if ! plan209_has_subfact "${label}" "1" 2>/dev/null &&
               ! plan209_has_subfact "${label}" "0" 2>/dev/null; then
              http_pass=false
              break
            fi
          done
          # http-command-exit and http-status must equal exactly "0" and "200".
          if ! plan209_has_subfact "http-command-exit" "0"; then
            http_pass=false
          fi
          if ! plan209_has_subfact "http-status" "200"; then
            http_pass=false
          fi
          for label in http-fixture-observed http-no-clearnet-fallback http-policy-retained \
                       ...
                       http-no-unknown-peer; do
            if ! plan209_has_subfact "${label}" "1"; then
              http_pass=false
              break
            fi
          done
          ...
          for label in http-body-digest http-request-digest http-multipacket-digest; do
            file=""
            for f in "${PLAN209_EVIDENCE_FILES[@]}"; do
              if [[ -f "${f}" ]]; then
          ...
          # Plan 209 §11 — required IRC subfacts.
          irc_pass=true
          for label in irc-driver-exit irc-registration-welcome irc-ping-pong-roundtrip \
                       ...
                       irc-privacy-hostname-rewrite irc-production-remote-outbound \
                       irc-production-remote-inbound irc-local-coowned-not-used \
                       irc-clean-resource-baseline irc-no-unknown-peer; do
            value=""
            for f in "${PLAN209_EVIDENCE_FILES[@]}"; do
              if [[ -f "${f}" ]]; then
                value=$(awk -F'\t' -v want="${label}" '$1 == want {print $2; exit}' "${f}" 2>/dev/null || true)
          ...
          if [[ "${http_pass}" != true || "${irc_pass}" != true ]]; then
            record "remote-independent-http-eepsite" failed \
              "Plan 209 product-only remote HTTP eepsite guard returned rc=${plan209_rc} without the documented Plan 209 §11 subfacts (http_pass=${http_pass} irc_pass=${irc_pass}; see plan209-remote-application.log + driver-evidence.tsv)"
            record "remote-independent-irc-service" failed \
              "Plan 209 product-only remote IRC service guard returned rc=${plan209_rc} without the documented Plan 209 §11 subfacts (http_pass=${http_pass} irc_pass=${irc_pass}; see plan209-remote-application.log + driver-evidence.tsv)"
          else
            record_guarded "remote-independent-http-eepsite" \
              "Plan 209 product-only remote HTTP eepsite: real system curl exit=0 + status=200 + body digest matches the loopback HTTP fixture, with Plan 208 operation-derived counters observed (see plan209-remote-application.log + driver-evidence.tsv)" \
              0
            record_guarded "remote-independent-irc-service" \
              "Plan 209 product-only remote IRC service: real exact-pinned jaraco/irc public API exit=0 + welcome + ping/pong + PRIVMSG round-trip + CTCP/DCC policy + privacy rewrite, with Plan 208 operation-derived counters observed (see plan209-remote-application.log + driver-evidence.tsv)" \
              0
          fi
        fi
      fi
  ...
  record_blocked "remote-independent-http-eepsite" \
    "not attempted in the local-only lane; run the full lane for the §6 + Plan 209 qualification"
  record_blocked "remote-independent-irc-service" \
    "not attempted in the local-only lane; run the full lane for the §6 + Plan 209 qualification"
  record_blocked "m10-remote-destination-streaming-composition" \
    "not attempted in the local-only lane; run the full lane for the §6 + Plan 202 + Plan 209 qualification"
fi
