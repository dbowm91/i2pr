#!/usr/bin/env bash
# Plan 185 — run the exploratory tunnel lane end-to-end.
#
# Local rows execute the focused daemon-owned coordinator +
# liveness scheduler suites. The external row provisions one
# ephemeral exact-pinned i2pd 2.61.0 process on loopback with
# transit enabled (the only Plan 184 setting that changes for
# Plan 185) and runs the single fail-closed driver through its
...
# target/interop; set I2PR_M6_TUNNEL_EVIDENCE_DIR to retain it
...
EVIDENCE_DIR="${I2PR_M6_TUNNEL_EVIDENCE_DIR:-${REPO_ROOT}/target/interop/m6-tunnel-evidence}"
...
I2PD_PORT="${I2PR_I2PD_PORT:-43884}"
I2PR_PORT="${I2PR_SSU2_TUNNEL_PORT:-44082}"
DRIVER_TIMEOUT="600s"

mkdir -p "${EVIDENCE_DIR}"
SCRATCH="$(mktemp -d -t i2pr-m6-plan185.XXXXXX)"
...
if [[ ! -f "${I2PD_CACHE}/source-revision.txt" ]] ||
  ...
  echo "i2pd cache has no verified Plan 185 source revision" >&2
  echo "run scripts/interop/fetch-ssu2-reference.sh --rebuild first" >&2
  exit 1
fi
...
# Plan 185 differs from Plan 184 only in `notransit = false` so the
# reference accepts our one-hop exploratory builds. All other
...
echo "==> local Plan 185 rows (coordinator + liveness scheduler suites)"
LOCAL_LOG="${EVIDENCE_DIR}/local-exploratory-build.log"
: > "${LOCAL_LOG}"
local_rc=0
cargo test --locked -p i2pr-daemon --test exploratory_build_unit -- \
  --test-threads=1 >>"${LOCAL_LOG}" 2>&1 || local_rc=$?
record_guarded "local-exploratory-build-unit" \
  "coordinator state-management unit rows (cargo test -p i2pr-daemon --test exploratory_build_unit)" \
  "${local_rc}"

LIVE_LOG="${EVIDENCE_DIR}/local-exploratory-build-live.log"
: > "${LIVE_LOG}"
live_rc=0
cargo test --locked -p i2pr-daemon --test exploratory_build_live -- \
  --test-threads=1 >>"${LIVE_LOG}" 2>&1 || live_rc=$?
record_guarded "local-exploratory-build-live" \
  "two-daemon-pair + simulated remote rows (cargo test -p i2pr-daemon --test exploratory_build_live)" \
  "${live_rc}"
...
echo "==> external exploratory tunnel lane against exact-pinned i2pd (explicit ignored selection)"
...
if I2PD_ROUTER_INFO="${I2PD_RI}" \
   ...
   exploratory_tunnels_against_i2pd -- --ignored --exact --nocapture --test-threads=1 \
   >>"${DRIVER_LOG}" 2>&1; then
  driver_rc=0
else
  driver_rc=$?
...
m6_row "external-outbound-build-emitted" "outbound-build-emitted" \
  "real one-hop outbound build emitted through Plan 185 coordinator"
m6_row "external-outbound-installed" "outbound-installed" \
  "outbound exploratory tunnel installed through existing pool/registry"
m6_row "external-inbound-build-emitted" "inbound-build-emitted" \
  "real one-hop inbound build emitted through Plan 185 coordinator"
m6_row "external-inbound-installed" "inbound-installed" \
  "inbound exploratory tunnel installed through existing pool/registry"
m6_row "external-liveness-first-test" "liveness-first-test" \
  "creator-side liveness scheduler first test succeeds"
...
evidence = {
    "schema": "i2pr-m6-tunnel-v1",
    "timestamp_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    "i2pr_commit": commit,
    "os_image": platform.platform(),
    "rust_toolchain": rustc,
    "execution_lane": "m6-tunnel-external",
    ...
    "i2pd": {
        ...
        "role": "mandatory independent Short Tunnel Build reference, unmodified",
        "transit": "notransit=false (Plan 185 only)",
    },
    "driver_evidence_keys": driver_keys,
    "results": rows,
    "m6_tunnel": "passed-via-i2pd-2.61.0" if all(
    ...
    "known_limitations": [
        ...
        "no destination LeaseSet2 or Streaming claim; Plan 186 owns NetDB lookup",
...
with (out / "evidence.md").open("w", encoding="utf-8") as stream:
    stream.write("# Plan 185 exploratory tunnel evidence (local suites + independent i2pd matrix)\n\n")
    ...
    stream.write("- i2pd profile: `notransit=false` (Plan 185 only)\n\n")
...
if [[ "${REQUIRED_FAILED}" -ne 0 ]]; then
  echo "Plan 185 tunnel lane failed; sanitized evidence: ${EVIDENCE_DIR}" >&2
  exit 1
fi
echo "Plan 185 tunnel lane passed; sanitized evidence: ${EVIDENCE_DIR}"
