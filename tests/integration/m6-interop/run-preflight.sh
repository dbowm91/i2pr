#!/usr/bin/env bash
# Plan 184 — run the authenticated I2NP preflight lane end-to-end.
#
# Local rows execute the focused daemon-owned SSU2 suites
# (config strict profile, dispatcher classification, delivery seams,
# shutdown baselines). The external row provisions one ephemeral
# exact-pinned i2pd 2.61.0 process on loopback and runs the single
# fail-closed driver
# (crates/i2pr-daemon/tests/ssu2_daemon_preflight.rs) through its
# explicit `--ignored --exact` selection, deriving each row from the
# driver exit status plus its own sanitized evidence keys.
#
# The lane is unprivileged and loopback-only. Required failures make
# this script fail. Sanitized evidence defaults below
# target/interop; set I2PR_M6_EVIDENCE_DIR to retain it elsewhere.
# Private router keys stay in the ephemeral scratch directory and are
# never copied to evidence.

set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
EVIDENCE_DIR="${I2PR_M6_EVIDENCE_DIR:-${REPO_ROOT}/target/interop/m6-preflight-evidence}"
...
I2PD_PORT="${I2PR_I2PD_PORT:-43883}"
I2PR_PORT="${I2PR_SSU2_PORT:-44081}"
DRIVER_TIMEOUT="600s"

mkdir -p "${EVIDENCE_DIR}"
SCRATCH="$(mktemp -d -t i2pr-m6-plan184.XXXXXX)"
...
if [[ ! -f "${I2PD_CACHE}/source-revision.txt" ]] ||
  ...
  echo "i2pd cache has no verified Plan 184 source revision" >&2
  echo "run scripts/interop/fetch-ssu2-reference.sh --rebuild first" >&2
  exit 1
fi
...
echo "==> local Plan 184 rows (daemon-owned SSU2 suites)"
LOCAL_LOG="${EVIDENCE_DIR}/local-daemon-preflight.log"
: > "${LOCAL_LOG}"
local_rc=0
cargo test --locked -p i2pr-daemon --test ssu2_daemon_preflight -- \
  --test-threads=1 --skip ssu2_daemon_preflight_against_i2pd >>"${LOCAL_LOG}" 2>&1 || local_rc=$?
record_guarded "local-daemon-preflight" \
  "daemon graph + dispatcher + delivery + shutdown rows (cargo test -p i2pr-daemon --test ssu2_daemon_preflight --skip external)" \
  "${local_rc}"

LIB_LOG="${EVIDENCE_DIR}/lib-router-i2np.log"
: > "${LIB_LOG}"
lib_rc=0
cargo test --locked -p i2pr-daemon --lib router_i2np -- \
  --test-threads=1 >>"${LIB_LOG}" 2>&1 || lib_rc=$?
record_guarded "lib-router-i2np" \
  "dispatcher/delivery/config mapping unit rows (cargo test -p i2pr-daemon --lib router_i2np)" \
  "${lib_rc}"

echo "==> external preflight against exact-pinned i2pd (explicit ignored selection)"
...
if I2PD_ROUTER_INFO="${I2PD_RI}" \
   ...
   cargo test --locked -p i2pr-daemon --test ssu2_daemon_preflight \
   ssu2_daemon_preflight_against_i2pd -- --ignored --exact --nocapture --test-threads=1 \
   >>"${DRIVER_LOG}" 2>&1; then
  driver_rc=0
else
  driver_rc=$?
...
m6_row "external-i2np-outbound" "outbound-accepted" \
  "benign control traverses i2pr -> i2pd through send_i2np"
m6_row "external-i2np-inbound" "inbound-dispatched" \
  "authenticated inbound traverses i2pd -> i2pr and reaches the central dispatcher"
m6_row "external-malformed-bounded" "malformed-bounded" \
  "malformed/expired/oversized inbound rejected boundedly"
m6_row "external-resource-baseline" "resource-sessions-closed" \
  "stopping either peer releases session/queue/socket/dispatch resources"
...
evidence = {
    "schema": "i2pr-m6-preflight-v1",
    "timestamp_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    "i2pr_commit": commit,
    "os_image": platform.platform(),
    "rust_toolchain": rustc,
    "execution_lane": "m6-preflight-external",
    ...
    "results": rows,
    "m6_preflight": "passed-via-i2pd-2.61.0" if all(
        row["status"] == "passed" for row in rows
    ) else "failed",
    "known_limitations": [
        "daemon-owned SSU2 activation and dispatch only; no tunnel/NetDB/Streaming claim",
        "direct loopback SSU2 session evidence only; no public I2P participation",
        "no NetDB lookup/publication over external peers; no destination/Streaming mixed-router claim",
        "no M10 remote-service claim; Plan 185 owns the first Short Tunnel Build",
    ],
}
...
with (out / "evidence.md").open("w", encoding="utf-8") as stream:
    stream.write("# Plan 184 preflight evidence (local suites + independent i2pd matrix)\n\n")
...
if [[ "${REQUIRED_FAILED}" -ne 0 ]]; then
  echo "Plan 184 preflight lane failed; sanitized evidence: ${EVIDENCE_DIR}" >&2
  exit 1
fi
echo "Plan 184 preflight lane passed; sanitized evidence: ${EVIDENCE_DIR}"
