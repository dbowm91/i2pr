...
next_product_layer = m6-java-second-family-qualification (Plan 196 + Plan 197 proven; external-session-established-java flipped failed -> passed on commit 5273768; resume Plan 194 §5.3 tunnel-over-tunnels + §5.4(b)/(c) bidirectional destination delivery + §5.5 Streaming qualification)
...
- Plan 194 is the M6 Java I2P second-family qualification that resumes now: §5.3 tunnel-over-tunnels + §5.4(b)/(c) bidirectional destination delivery + §5.5 Streaming qualification against the proven controlled Java topology + authenticated SSU2 preflight + STYLE=RAW SAM bridge. The seven §11 stop-provenance install-dependent rows (`external-outbound-tunnel`, `external-inbound-tunnel`, `external-lease-lookup-tunnel`, `external-ls2-publication-tunnel`, `external-destination-outbound`, `external-reference-received`, `external-destination-inbound`) flip from `blocked` to `passed` as §5.3 lands. Plan 195 stays blocked on Plan 194 closing the two-family M6 criterion.
