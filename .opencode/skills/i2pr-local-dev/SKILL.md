...
plan_201 = blocked-by-plan218-fresh-external-classification (Plan 217 closed the harness/evidence corrective; Plan 218 owns the fresh exact-head external run that records the corrected-harness terminal `P200-*` classification and flips the seven §11 stop rows `blocked → passed`; Plan 217 supersedes the prior `in-progress-branch-c-d-attempt-blocked-on-java-loopback-peer-profile-scoring` token — the Branch C/D three-router topology + 1-hop + zero-hop attempts remain as the documented Plan 201 narrow-attempt narrative; the destination driver's `LeaseStoreIngestOutcome::Completed` reached before the Plan 216 panic is the reason the Plan 216 "no LS2 reply arrives" interpretation is no longer authoritative)
...
plan_218 = ready-m6-java-second-family-final-qualification (next executable plan; runs the corrected `bash tests/integration/m6-interop/run-java.sh` harness end-to-end against the exact-pinned Java I2P 2.13.0 cache; records the fresh terminal `P200-*` classification; flips the seven §11 stop rows `blocked → passed`; on success, M6 closes and Plan 204 becomes dependency-ready; on a genuine stock-Java direct-I2CP boundary, retained Plan 205 may be reactivated)
...
next_executable_plan = 218-m6-java-second-family-final-qualification (Plan 217 closed the harness/evidence corrective; Plan 201 stays blocked on Plan 218's fresh exact-head external run; M10 Plans 212-215 passed; Plan 204 remains blocked per `plans/closure/service-tunnels/204-status.md`)
...
m6_java_second_family_qualification = public-client-diagnostic-evidence-passed-via-plan200-and-branch-g-framework-landed-via-plan201-and-harness-corrective-passed-via-plan217 (Plan 199 two-router bootstrap passes; Plan 200 closed the diagnostic/evidence side with sanitized lifecycle keys and one terminal `P200-{A..H}` classification per run; Plan 201 landed the Branch G corrective framework with eleven typed counters on `DestinationTunnelCounters` + the public `note_lookup_boundary` helper; Plan 217 closed the harness/evidence corrective (transfer-once invariant, positive/negative evidence split, relative Java NetDB dir, disjoint streaming namespace, sub-run selector); the corrected harness is the Plan 218 input; downstream M6 rows stay blocked pending the Plan 218 fresh exact-head external run consuming the corrected `P200-*` classification)
