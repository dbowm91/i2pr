...
plan_198 = blocked-public-java-client-leaseset2-publication (public helpers connect and Rust driver processes pass, but mandatory lookup/delivery/Streaming rows remain blocked)
...
- Plan 194 retains the Java SAM-bridge publication-boundary evidence. Plan 198 replaces the counted Java service with public `I2PClient`/`I2PSession` and `I2PSocketManager` helpers, but the exact-pinned Java 2.13.0 controlled router still does not return the public-client LeaseSet2 to the real i2pr lookup path. The mandatory lookup, destination-delivery, and Streaming rows remain blocked with fail-closed provenance; Plan 195 is still gated.
...
i2pr session layer stays classical X25519 only, i2pr publication
  path stays pq-free, ML-KEM not implemented. Plan 198 is the current
