...
milestone10_remote_service_interop = not-yet-passed (Plan 213 closed the generic router-backed Direction A+B external proof twice on exact commit ef59fb3; retained Plans 202/206/207/208/209 structure stays green; Plan 210 over-strong closure superseded by Plan 212; Plan 211 is retained-superseded-for-final-evidence-by-Plan 214 — see plans/214-status.md; Plan 215 source-side corrective landed — deterministic `write_plan214_tunnels_conf` `printf`-based writer + `validate_plan214_tunnels_conf` pre-launch sanity gate + sanitized `plan214-reference-tunnel-config-sanity` evidence row + focused `test-plan215-tunnels-conf.sh` contract test + §29 static checker; the next action is to dispatch the hosted `full` lane twice sequentially on one immutable Plan 215 corrective SHA — see plans/215-status.md)
milestone10_final_acceptance = not-yet-closed (Plan 213 closed the generic A+B external proof twice on exact commit ef59fb3; Plan 214 source landed; Plan 215 source-side corrective landed — see plans/215-status.md; the hosted `full` lane that provisions i2pd with peers is the authoritative external qualification surface that flips the two `remote-independent-*` rows from `blocked` to `passed`; Plan 201's terminal `P200-{A..H}` classification + narrow corrective still gates the cross-milestone Java second-family closure)

next_product_layer = plan214-hosted-double-pass (Plan 213 passed; Plan 214 source landed: black-box v214 driver + pumped subprocesses + fresh-seq target facts + standalone runner + §28 checker; Commits E-F own local exact-head validation + two consecutive hosted `full` runs with `P214-N-passed`; the remaining M10 work after that is the Plan 201 Java corrective plus the Plan 204 final closure transition)
...
plan_211 = retained-source-harness-superseded-for-final-evidence-by-plan214 (source harness retained; final counted evidence owned by the Plan 214 v214 driver + standalone `run-plan214-applications.sh` runner — see plans/214-status.md; **no private key material crosses the trust boundary**)
...
m10_remote_application_interop = locally-passed-once (Plan 214 P214-N on the Commit E tree: typed remote-mirror resolution seam, transparent i2pd server tunnel with tap attribution, neutral-shape privacy proof; Plan 215 source-side corrective landed; hosted double-pass pending on the Plan 215 corrective SHA)
next_m10_application_plan = plan215-hosted-double-pass (Plan 213 passed; Plan 214 local pass proven on the Commit E tree; Plan 215 source-side corrective landed — deterministic `write_plan214_tunnels_conf` writer + `validate_plan214_tunnels_conf` pre-launch sanity gate + focused contract test + §29 static checker; the next action is to dispatch the hosted `full` lane twice sequentially on one immutable Plan 215 corrective SHA and require `P213-N-passed` + `P214-N-passed` on both runs; Plan 204 owns the docs/CI/evidence-authority normalization pass; Plan 201 owns the cross-milestone Java second-family closure)
