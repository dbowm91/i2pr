...
next_product_layer = plan213-external-qualification-then-plan211-requalification (Plan 213 Commits A-D landed — generic A/B application driver with real TCP I/O, harness-only SAM STREAM fixture, standalone fail-closed runner, §27 checker, router-generic workflow lane; hosted double-run pending; Plan 212 source closure landed; external generic A+B pending; Plan 210 over-strong closure superseded; Plan 210 retained the real per-service destination tunnel material + inbound Streaming corrective; Plan 211 ships the source-side closure with real enabled service specs + i2pd destination extraction + subfact emission + static-checker invariants; the remaining M10 work is the Plan 201 Java corrective and the dedicated M6 interop lane that flips the two aggregate rows from blocked to passed)
