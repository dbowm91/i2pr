---
name: i2pr-local-dev
description: Work on the local product path of the i2pr Rust I2P router — Milestone 6 destinations/garlic/LeaseSet2/Streaming, Milestone 7 SAM 3.1, Milestone 8 SSU2, Milestone 9 I2CP, and Milestone 10 service tunnels execution. Plans 155–197 retain their bounded passed evidence; Plan 193 closed the i2pd M6 Streaming family, while Plan 194 retained the Java SAM publication boundary. Plan 199 is the unified M6/M10 closure attempt: its two-router ordinary-RouterInfo bootstrap passes, but the exact-pinned Java 2.13.0 public-client LeaseSet2 remains network-invisible and the M10 manager has no real remote-router transport path. Mandatory Java/M10 remote rows remain blocked and neither milestone is claimed.
---
...
plan_198 = blocked-public-java-client-leaseset2-publication (superseded into Plan 199; mandatory lookup/delivery/Streaming rows remain blocked)
plan_199 = blocked-java-public-client-leaseset2-and-m10-remote-transport (two-router bootstrap passes; client LeaseSet2 remains network-invisible and the M10 manager has no real remote-router path)
...
- Plan 194 retains the Java SAM-bridge publication-boundary evidence. Plan 199 replaces the counted Java service with public `I2PClient`/`I2PSession` and `I2PSocketManager` helpers plus a two-router ordinary-RouterInfo bootstrap. The exact-pinned Java 2.13.0 controlled routers still do not return the public-client LeaseSet2 to the real i2pr lookup path. The mandatory lookup, destination-delivery, and Streaming rows remain blocked with fail-closed provenance; Plan 195 is still gated.
...
i2pr session layer stays classical X25519 only, i2pr publication
  path stays pq-free, ML-KEM not implemented. Plan 199 is the current
  executable corrective: its public `I2PClient`/`I2PSession` and
  `I2PSocketManager` helpers connect, but the exact-pinned Java router has
  not returned their LeaseSet2 to the real i2pr lookup path. Mandatory
  lookup/delivery/Streaming rows remain blocked and Plan 195 stays gated.
  See `plans/198-status.md`, `plans/196-status.md`, and
  `plans/197-status.md`.
