...
`Ssu2RouterAddress::parse`). Plan 197 has landed the narrow PQ
...
path stays pq-free, ML-KEM not implemented. The Plan 196 external
lane now re-runs and the
`external-session-established-java` row flips `failed` → `passed`;
Plan 196 then flips to `passed-m6-java-controlled-first-run-topology-corrective`
and Plan 194 resumes the Java second-family qualification. See
