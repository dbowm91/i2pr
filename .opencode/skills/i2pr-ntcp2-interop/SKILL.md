...
description: Operate, diagnose, or extend the repository's Plan 038/040/041/043/044/045/052/053/054/055/058/059/062/063/064/065/067/068/074/075/076/077/078/080/081/082/083/084/085/086/087/088/090/091/092/093/094/099/100 host-side Ubuntu 24.04 reference-router NTCP2 interoperability harness (Plan 099/100 closed as protocol-defect-localized; Plan 095 historical). Plan 101 corrects the daemon NTCP2 activation boundary; normal-daemon NTCP2 is disabled. Plan 115 Emissary Q0 construction + native OBEP reply has passed locally with the canonical production I2NP bridge (`ShortBuildI2npBridge` in `crates/i2pr-tunnel/src/bridge.rs`); Q1/Q2/qualified external delivery still pending. Plan 117 closed per Plan 118 as `closed-for-progression-with-evidence-gap` (local Phase G passed; corrected native Emissary reference test rejected the pinned reference's request-prefixed reply during strict i2pr Mapping decoding). The harness includes the Plan 085-088 host-loopback development execution roadmap (historical), the Plan 090 i2pd RouterInfo and pre-TCP classification correction, the Plan 091 i2pd Noise-handshake preconditions, the Plan 092 forward-handshake evidence integrity (superseded by Plan 093), the Plan 093 Plan 087 forward data-phase and reference-observer closure, the Plan 094 Plan 093 completion pass and Plan 087 -> Plan 088 handoff, the Plan 099 Milestone 3 interop exit and router buildout, the Plan 100 one-time exit-gate cleanup and router handoff. Use when an agent is asked to read or reproduce the historical harness surface, run a bounded interop profile, prepare or validate reference routers, add or modify a scenario, or validate evidence. The active development interop lane is closed; NTCP2 remains experimental and non-advertised. Do not activate Plan 072 or build a general Emissary lane unless Plan 088 records `decision = ambiguous-reference-divergence` with one exact wire-stage question. Plan 119 closed as `passed-leaseset2-protocol-foundation`; Plan 120 closed as `passed-destination-lifecycle-and-pools` and lands the first `i2pr-client` destination runtime (local identity, dedicated tunnel pools, signed Standard LeaseSet2, lifecycle, bounded local payloads, registry); Plan 121 closed as `passed-ecies-destination-session-layer`; Plan 126 closed as `passed-ecies-destination-ratchet-corrective-foundation` and rewrote that layer to the normative I2P ECIES-X25519-AEAD-Ratchet contract (bound NS with Alice's derived public key, SessionReplyTags NSR window, Noise Split k_ab/k_ba tag sets + AttachPayloadKDF, canonical tag/key index alignment, ES tag-AD AEAD; corrected primitives in `crates/i2pr-crypto/src/ecies.rs`, corrected manager in `crates/i2pr-client/src/session.rs`, trajectories in `plan121_trajectory.rs`/`plan126_trajectory.rs`, provenance in `specs/references/ecies-destination-ratchet.md`; Plan 127 then bound the pairing to resolved Destinations with bundled-LS2 sender binding, the `PlannedOutboundForm` state machine, and production reverse routing through `install_remote_lease_set2`, closing as `passed-destination-session-routing-final-closure` per `plans/127-status.md`); Plan 122 closed as `passed-corrected-local-destination-routing` and lands the local destination routing pipeline (Plan 122 `compose_outbound_delivery` / `DestinationDispatcher`, daemon `NetDbSeam` `begin_lease_set2_lookup` / `ingest_lease_set2_response` / `cancel_lease_set2_lookup`); Plan 124 closed as `passed-plan122-corrective-closure` and corrects the Plan 122 composition defect where `compose_outbound_delivery` retained an ECIES Garlic envelope but fed the plaintext inner I2NP `Data` envelope into the outbound tunnel role — the corrected composition wraps the encrypted envelope in an `I2npBody::Garlic` carrier and feeds the standard-encoded I2NP Garlic message bytes into the outbound tunnel data plane (`crates/i2pr-client/src/routing.rs`, `crates/i2pr-client/tests/plan124_trajectory.rs`); Plan 128 closed as `passed-streaming-wire-protocol-corrective-closure`, correcting the Streaming packet wire format to the current I2P specification and restoring Plan 123 as `passed-corrected-streaming-wire-local` (provenance in `specs/references/streaming-packet-wire.md`); Plan 125 is now superseded-by-final-corrective-closure. Plan 129 is now `superseded-by-plan130-final-gate`. Plan 130 closed as `superseded-by-plan131-final-local-correctness-gate` (historical evidence retained in `plans/130-status.md`). Plan 133 remains successful historical evidence; Plan 134 is the current Milestone 6 authority (`passed-milestone6-recv-window-ack-ceiling-closure`, `plans/134-status.md`): production Elligator branch randomization via the reviewed `elligator2 = 0.1.0` primitive in `crates/i2pr-crypto/src/ecies.rs` (`curve25519-elligator2 = 0.1.0-alpha.2` retired because its `RFC9380` branch choice was deterministic rather than caller-randomizable and its `Randomized` mode rotated the derived X25519 public key; provenance in `specs/references/elligator2-production-representation.md`), corrected Streaming sequence space in `crates/i2pr-client/src/streaming/` (SYN/response seq 0, first application packet seq 1, plain-ACK control form never delivered), semantic flag-driven ACK presence where `ackThrough == 0` is valid, NACK-aware cumulative ACKs retaining explicitly NACKed packets, receiver ack views per Java `MessageInputStream.updateAcks`, synchronous coalescing delayed standalone ACKs via `poll_acks(now_ms)` on the 750 ms reference default with piggyback suppression, wire destination_port listener authority with exact/wildcard-port-0 matching and typed `NoMatchingListener`/`PortTupleMismatch`, persistent tunnel duplicate windows with typed `DuplicateCell` replay rejection (`crates/i2pr-tunnel/src/roles.rs`), independent three-layer replay separation (tunnel duplicate window, consumed ECIES session tag, fresh-ECIES-reseal Streaming sequence), connection-owned I2P port tuple asserted on every established `send_data`/`send_close`/`send_reset`, source-port `0` as valid I2P "unspecified", side-effect-free oversized `send_data` rollback, eleven full-stack Plan 130 trajectories in `crates/i2pr-client/tests/plan130_trajectory.rs`, and seven Plan 131 trajectories in `crates/i2pr-client/tests/plan131_trajectory.rs`. Milestone 6 local product is passed under corrected local-correctness semantics; interoperability is not claimed; corrective Milestone 6 planning stops and SAM baseline planning (Milestone 7) is next. External acceptance debt is tracked separately under the Milestone 6 roadmap. The companion skills `i2pr-rootless-sandbox` and `i2pr-multipass-recovery` cover the Plan 046 sealed-namespace lane and the Plan 048/049/050/051 recovery lane.
---
...
next_router_construction_plan        = SAM baseline planning (Milestone 7; Milestone 6 closed by Plan 134)
```
...
tunnel pools, signed Standard LeaseSet2, lifecycle, bounded local
 payloads, registry). Plan 121 closed as
 `passed-ecies-destination-session-layer` and lands the first real
 ECIES-X25519-AEAD-Ratchet destination Garlic/session layer.
 Plan 122 closed as `passed-corrected-local-destination-routing`
 and Plan 124 closed as `passed-plan122-corrective-closure` (the
 canonical ECIES Garlic carrier through the outbound tunnel data
 plane; `OutboundDeliveryPlan::garlic_i2np_bytes` is the canonical
 carrier; `DestinationDispatcher::bind_destination_hash` enforces
 the `DestinationId` → `DestinationHash` binding so the dispatcher
 fails closed on `UnknownDestination` without trial-decryption).
 Plan 125 closed as `passed-milestone6-local-corrective-closure`
 and restored Plan 123 to `passed-corrected-minimal-streaming-local`;
 Plan 125 replaces the custom `ClientPayload` framing with the
 canonical RFC 1952 gzip wire format and restores the real
 SYN / SYN-response lifecycle. Plan 126 closed as
 `passed-ecies-destination-ratchet-corrective-foundation` and rewrote
 that layer to the normative I2P ECIES-X25519-AEAD-Ratchet contract
 (bound NS with Alice's derived public key, SessionReplyTags NSR
 window, Noise Split k_ab/k_ba tag sets + AttachPayloadKDF,
 canonical tag/key index alignment, ES tag-AD AEAD; provenance in
 `specs/references/ecies-destination-ratchet.md`). Plan 127 closed
 as **`passed-destination-session-routing-final-closure`**
 (`plans/127-status.md`): a fresh bound New Session always bundles
 the local current signed Standard LeaseSet2, the dispatcher binds an
 accepted NS only after validating exactly one bundled sender LS2
 under its own contained Destination hash with a type-4 key equal to
 the authenticated static key (`MissingSenderLeaseSet2` /
 `SenderKeyMismatch` rejections otherwise), the session manager owns
 the `PlannedOutboundForm` state machine (retained NSR context →
 Existing Session → fresh NS) so the first reply rides the retained
 reply context, reverse routing uses the explicit
 `DestinationRouting::install_remote_lease_set2` typed handoff under
 a new `MAX_ACTIVE_REMOTES` ceiling, and the master trajectory
 `plan_127_master_trajectory_ns_nsr_es_bidirectional_exact_once`
 drives two destinations through real tunnel roles with exact-once
 ES traffic in both directions. Plan 128 closed as
 `passed-streaming-wire-protocol-corrective-closure` per
 `plans/128-status.md`; it corrects the Streaming packet wire format
 below the retained RFC 1952 gzip layer (normative flag map, no option
 TLVs, payload-only 2-byte MAX_PACKET_SIZE defaulting to 1730, raw
 final signatures from signing-key context, canonical zeroed-placeholder
 preimage, Proposal 164 replay NACKs on the initial SYN only, retained
 peer signing key for CLOSE/RESET verification without FROM,
 min-of-advertisements negotiation; provenance in
 `specs/references/streaming-packet-wire.md`) and restores Plan 123 as
 `passed-corrected-streaming-wire-local`; Plan 125 becomes
 `superseded-by-final-corrective-closure`. Plan 129 closed as
 **`passed-milestone6-integrated-local-product-gate`**
 (`plans/129-status.md`), a superseded historical Milestone 6
 local-product gate (now superseded by Plan 131/132/133/134 — Plan 133
 remains successful historical evidence and Plan 134 is the current
 Milestone 6 local closure authority as
 `passed-milestone6-recv-window-ack-ceiling-closure`):
 one combined runtime-neutral outbound/inbound
 `StreamingDestinationAdapter` (outbound bound against
 `MAX_CLIENT_PAYLOAD_BYTES`, single canonical Data-envelope owner;
 inbound standard-I2NP -> Data -> gzip -> protocol-6 -> I2P ports ->
 Streaming dispatch with typed `UnsupportedProtocol` outcome),
 one combined runtime-neutral outbound/inbound
 `StreamingDestinationAdapter` (outbound bound against
 `MAX_CLIENT_PAYLOAD_BYTES`, single canonical Data-envelope owner;
 inbound standard-I2NP -> Data -> gzip -> protocol-6 -> I2P ports ->
 Streaming dispatch with typed `UnsupportedProtocol` outcome),
 integrated retransmission (`poll_retransmits`), cumulative end-to-end
 ACKs clearing tracked packets, ordered delivered-byte surfacing after
 reorder (`drain_delivered`), CLOSE/RESET completion policy where no
 side closes merely because it queued a CLOSE, and twelve deterministic
 tests in `crates/i2pr-client/tests/plan129_trajectory.rs`.
 The Plan 131 corrections (production Elligator branch randomization,
 three-layer replay separation, connection-owned I2P ports on every
 established send API, side-effect-free oversized `send_data` rollback)
 and the Plan 132 corrections (strict Elligator2 receive-domain
 validation, three layer-isolated replay trajectories, transactional
 `send_data` / `send_close` / `send_reset` ordering) are layered on
 top of the Plan 129 surface. The Plan 133 closure rewrote B2/B3 of
 `plan132_trajectory.rs` to retain the actual first delivered ES
 envelope and corrected the Elligator reference note for the i2pd
 equality-boundary executable distinction.
...
roadmap's external acceptance debt ledger. The next product layer is
